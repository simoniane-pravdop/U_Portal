    ...
    <main className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
      ...
      <section className="workspace">
        <div className="content" id="main-content">
          ...
          {view === "coordination" && <CoordinationView payload={payload} userById={userById} select={(id) => { const node = payload.nodes.find((item) => item.id === id); if (node) openNodeInWork(node); }} open={(node) => { setSelectedId(node.id); setModal("coordination"); }} />}
  ...
  return <>
    ...
    <div className={`tree-workbench ${treeCompact ? "compact-tree" : ""} mobile-pane-${mobilePane}`} style={{ "--tree-nav-width": `${treeWidth}px` } as React.CSSProperties}>
      ...
      {selected ? <section className="node-detail">
        ...
        {detailTab === "passport" && <WorkCardDescription key={selected.id} node={selected} payload={payload} userById={userById} expanded />}
...
function WorkCardDescription({ node, payload, userById, expanded = false }: { node: WorkNode; payload: PortalPayload; userById: (id: string) => PortalUser | undefined; expanded?: boolean }) {
  ...
  return <details className={`work-card-description ${expanded ? "tree-passport" : ""}`} open={expanded}><summary><div><span>Паспорт робочої картки</span><strong>Опис картки</strong><small>{path.map((item) => item.code).join(" / ")} · {node.description || node.result}</small></div><i className="details-state" aria-hidden="true" /></summary><div className="work-card-description-body"><section><h4>Місце в структурі</h4><dl><dt>Шлях</dt><dd>{path.map((item) => `${item.code} · ${item.title}`).join(" → ")}</dd><dt>Рівень</dt><dd>{kindLabels[node.kind]}</dd><dt>Код і назва</dt><dd>{node.code} · {node.title}</dd></dl></section><section><h4>Результат і межі</h4><dl><dt>Опис</dt><dd>{value(node.description)}</dd><dt>Готовий результат</dt><dd>{value(node.result)}</dd><dt>Що не є результатом</dt><dd>{value(node.nonResult)}</dd><dt>Критерій приймання</dt><dd>{value(node.acceptanceCriteria)}</dd></dl></section><section><h4>Відповідальні</h4><dl><dt>Виконавець</dt><dd>{userById(node.assigneeId)?.name || "Не визначено"}</dd><dt>Ініціатор</dt><dd>{userById(node.acceptorId)?.name || "Не визначено"}</dd><dt>Учасники / фоловери</dt><dd>{participants || "Не визначено"}</dd></dl></section><section><h4>Строки та стан</h4><dl><dt>Плановий початок</dt><dd>{dateLabel(node.plannedStart)}</dd><dt>Фактичний початок</dt><dd>{dateLabel(node.actualStart)}</dd><dt>Дедлайн до</dt><dd>{dateLabel(node.plannedEnd)}</dd><dt>Прогноз завершення</dt><dd>{dateLabel(node.forecastEnd)}</dd><dt>Фактичне завершення</dt><dd>{dateLabel(node.actualEnd)}</dd><dt>Статус</dt><dd>{lifecycleLabels[node.lifecycle]} · {node.progress}%</dd><dt>Стан виконання</dt><dd>{healthLabels[node.health]}</dd>{node.healthComment && <><dt>Коментар до стану</dt><dd>{node.healthComment}</dd></>}<dt>Пріоритет</dt><dd>{priorityLabels[node.priority]}</dd><dt>Потрібне рішення</dt><dd>{node.decisionRequired ? "Так" : "Ні"}</dd></dl></section><section><h4>Умови виконання</h4><dl><dt>Спосіб початку</dt><dd>{startLabels[node.startMode]}</dd><dt>Обмеження повноважень</dt><dd>{value(node.authority)}</dd><dt>Ресурс</dt><dd>{value(node.resource)}</dd><dt>Контрольне місце</dt><dd>{value(node.controlPlace)}</dd><dt>Доступ</dt><dd>{"Усі активні користувачі порталу (перегляд)"}</dd><dt>Повторення</dt><dd>{node.recurrence?.enabled ? ({ weekly: "Щотижня", monthly: "Щомісяця", quarterly: "Щокварталу", yearly: "Щороку" }[node.recurrence.frequency]) : "Немає"}</dd>{node.kind === "cycle" && <><dt>Координація</dt><dd>{coordinationCadenceLabel(node)}</dd><dt>Перша координація</dt><dd>{dateLabel(node.coordinationStartDate || "")}</dd></>}</dl></section></div></details>;
}

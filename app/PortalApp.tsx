...
import { plannedEndForForecast } from "./lib/forecast-deadline";
import { compareNodeCodes } from "./lib/node-order";
import { nodeCreation } from "./lib/node-creation";
import { showActualMilestones } from "./lib/node-date-visibility";
import { projectTaskResults } from "./lib/project-results";
...
const kindLabels: Record<NodeKind, string> = { goal: "Стратегічна ціль", cycle: "Напрям зусиль", subcycle: "Проект", task: "Завдання" };
...
const roleLabels: Record<string, string> = {
  ...
  admin: "Адміністратор",
  goal_owner: "Керівник цілей",
  cycle_owner: "Керівник напрямів зусиль",
  coordinator: "Координатор",
  executor: "Виконавець",
  viewer: "Спостерігач",
};
function workFilterForUser(node: WorkNode | undefined, user: PortalUser): WorkFilter {
  if (!node) return "all";
  if (node.acceptorId === user.id) return "acceptance";
  if (node.assigneeId === user.id) return "action";
  return "all";
}
...
function cardHref(node: WorkNode, user: PortalUser) {
  return portalHref(hasWorkAccessForUser(node, user) ? "my" : "tree", node.id);
}
...
function coordinationAttentionReasons(payload: PortalPayload, node: WorkNode) {
  ...
  if ((payload.discussions || []).some((message) => message.nodeId === node.id && !message.deletedAt && !message.resolvedAt && message.kind === "issue")) reasons.add("Повідомлення про перешкоду без відповіді");
...
function buildAsanaDescription(payload: PortalPayload, node: WorkNode) {
  ...
  return [
    ...
    `Ініціатор: ${userName(node.acceptorId)}`,
...
function firstAvailableNumber(values: number[]) {
  const occupied = new Set(values.filter((value) => Number.isInteger(value) && value > 0));
  let candidate = 1;
  while (occupied.has(candidate)) candidate += 1;
  return candidate;
}

function nextCode(nodes: WorkNode[], parent?: WorkNode, excludeId = "") {
  const availableNodes = nodes.filter((node) => node.id !== excludeId);
  if (!parent) {
    const numbers = availableNodes.filter((node) => node.kind === "goal").map((node) => Number(node.code.match(/^S(\d+)$/i)?.[1] || 0));
    return `S${firstAvailableNumber(numbers)}`;
  }
  if (parent.kind === "goal") {
    const numbers = availableNodes.filter((node) => node.kind === "cycle").map((node) => Number(node.code.match(/^P(\d+)$/i)?.[1] || 0));
    return `P${firstAvailableNumber(numbers)}`;
  }
  const prefix = `${parent.code}.`;
  const numbers = availableNodes.filter((node) => node.parentId === parent.id && node.code.startsWith(prefix)).map((node) => Number(node.code.slice(prefix.length).match(/^(\d+)$/)?.[1] || 0));
  return `${prefix}${firstAvailableNumber(numbers)}`;
}
...
function blankNode(nodes: WorkNode[], parent: WorkNode | undefined, user: PortalUser, requestedKind?: NodeKind): WorkNode {
  ...
  return {
    ...
    acceptanceCriteria: "",
    assigneeId: user.id,
    acceptorId: parent?.acceptorId || user.id,
    participantIds: [],
    lifecycle: "draft",
...
function CreationNote({ node, payload }: { node: WorkNode; payload: PortalPayload }) {
  const creation = nodeCreation(node, payload.audit, payload.users);
  const timestamp = creation.at && Number.isFinite(Date.parse(creation.at))
    ? new Intl.DateTimeFormat("uk-UA", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit", timeZone: "Europe/Kyiv" }).format(new Date(creation.at))
    : "дата не збережена";
  return <span className="creation-note" title="Автор і фактичний час першого збереження картки (Київ)">Створив: <strong>{creation.name}</strong> · {timestamp}</span>;
}
...
export function PortalApp() {
  ...
  const mutate = useCallback(async (action: string, entityId: string, recipe: (state: PortalState) => void) => {
    ...
    try {
      for (let attempt = 0; attempt < 2; attempt += 1) {
        ...
        const body = (await response.json()) as PortalPayload & { error?: string; asanaDelivery?: { pending: number; errors: string[] } };
        if (response.ok) {
          applyPayload(body as PortalPayload);
          setNotice(body.asanaDelivery?.errors?.length ? "Зміни в порталі збережено, але передання в Asana очікує повторної спроби. Перевірте блок Asana у картці." : attempt ? "Зміни об’єднано з новішою редакцією та збережено" : "Зміни збережено", body.asanaDelivery?.errors?.length ? "error" : undefined);
          return true;
        }
  ...
  const canManage = ["owner", "admin", "goal_owner", "cycle_owner"].includes(payload.currentUser.role);
  const logout = async () => { await fetch("/api/auth/logout", { method: "POST" }); window.location.reload(); };
  const openNodeInWork = (node: WorkNode, focus: WorkFocus = null) => {
    setSelectedId(node.id);
    if (!hasWorkAccessForUser(node, payload.currentUser)) { setView("tree"); return; }
  ...
  const saveNode = async () => {
    ...
    if (!draftNode.assigneeId) errors.assigneeId = "Оберіть виконавця.";
    if (!draftNode.acceptorId) errors.acceptorId = "Оберіть ініціатора.";
    if (draftNode.kind !== "task" && draftNode.healthOverride && !draftNode.healthComment?.trim()) errors.healthComment = "Додайте пояснення ризику або блокера.";
    ...
    const previousNode = payload.nodes.find((node) => node.id === draftNode.id);
    if (previousNode && previousNode.code !== draftNode.code) {
      const branchIds = new Set(descendants(payload.nodes, draftNode.id).map((node) => node.id));
      const futureCodes = payload.nodes.filter((node) => branchIds.has(node.id) && node.id !== draftNode.id && node.code.startsWith(`${previousNode.code}.`)).map((node) => `${draftNode.code}${node.code.slice(previousNode.code.length)}`.toLowerCase());
      const conflict = payload.nodes.find((node) => !branchIds.has(node.id) && futureCodes.includes(node.code.toLowerCase()));
      if (conflict) {
        setNodeErrors({ code: `Перенумерація створить конфлікт із ${conflict.code}. Оберіть інше місце в дереві.` });
        setNotice("Не вдалося зберегти: коди нижчих рівнів перетнуться з наявною гілкою.", "error");
        return;
      }
    }
    const updated = { ...draftNode, plannedEnd: plannedEndForForecast(draftNode.plannedEnd, draftNode.forecastEnd), updatedAt: isoNow() };
    const ok = await mutate(exists ? `Оновлено ${draftNode.code}` : `Створено ${draftNode.code}`, draftNode.id, (state) => {
      ...
      if (index >= 0) {
        const previousCode = state.nodes[index].code;
        state.nodes[index] = updated;
        if (previousCode !== updated.code) {
          const branchIds = new Set(descendants(state.nodes, updated.id).map((node) => node.id));
          for (const child of state.nodes) if (child.id !== updated.id && branchIds.has(child.id) && child.code.startsWith(`${previousCode}.`)) child.code = `${updated.code}${child.code.slice(previousCode.length)}`;
        }
      }
  ...
  const submitAcceptance = (node: WorkNode, acceptorId: string, evidenceNote: string) => mutate(`Завершення ${node.code} передано на приймання`, node.id, (state) => {
    ...
    state.discussions.unshift({ id: crypto.randomUUID(), nodeId: node.id, authorId: payload.currentUser.id, recipientId: acceptorId, relatedType: "acceptance", relatedId: acceptance.id, text: `Виконавець завершив завдання та передав результат на приймання ініціатору: ${payload.users.find((user) => user.id === acceptorId)?.name || "не визначено"}. Критерій: ${node.acceptanceCriteria || "не визначено"}.${evidenceNote.trim() ? ` Докази: ${evidenceNote.trim()}` : ""}`, kind: "approval", createdAt: isoNow() });
  });
  ...
  const saveWorkUpdate = (node: WorkNode, update: Omit<WorkUpdate, "id" | "createdAt" | "createdBy" | "source">) => mutate(
    ...
    (state) => {
      ...
      target.plannedEnd = plannedEndForForecast(target.plannedEnd, update.forecastEnd);
      target.updatedAt = isoNow();
  ...
  const completeNode = async (node: WorkNode) => {
    ...
    if (live.kind === "task") {
      if (live.lifecycle === "acceptance") { setNotice("Завершення вже передано ініціатору на приймання", "error"); return false; }
    ...
    <main className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
      ...
      <aside className="sidebar">
        ...
        <nav aria-label="Основна навігація">
          {nav.map((item) => (
            <a key={item.id} href={portalHref(item.id)} aria-current={view === item.id ? "page" : undefined} className={`nav-item nav-${item.id} ${view === item.id ? "active" : ""}`} title={item.label} onClick={(event) => { if (!ordinaryLinkClick(event)) return; event.preventDefault(); if (item.id === "my") { setWorkEntryFilter("action"); setWorkEntryFocus(null); } setView(item.id); }}>
      ...
      <section className="workspace">
        <div className="content" id="main-content">
          {view === "dashboard" && <DashboardView data={data} payload={payload} userById={userById} healthFilter={healthFilter} setHealthFilter={setHealthFilter} select={(id) => { setSelectedId(id); setView("tree"); }} openWork={(id) => { const node = payload.nodes.find((item) => item.id === id); if (node) openNodeInWork(node); }} />}
          ...
          {view === "calendar" && <CalendarView payload={payload} userById={userById} openNode={(id) => { const node = payload.nodes.find((item) => item.id === id); if (node) openNodeInWork(node); }} />}
          {view === "tree" && (
            <TreeView
              ...
              openWork={(node) => openNodeInWork(node)}
              mutate={mutate} copyNodeLink={copyNodeLink} notify={setNotice}
            />
          )}
      ...
      {modal === "blocker" && selected && <BlockerModal node={selected} users={payload.users} currentUserId={payload.currentUser.id} notify={setNotice} close={() => setModal(null)} save={async (blocker) => { const ok = await mutate(`Додано блокер до ${selected.code}`, selected.id, (state) => { state.blockers.unshift(blocker); state.discussions.unshift({ id: crypto.randomUUID(), nodeId: selected.id, authorId: payload.currentUser.id, recipientId: blocker.escalationToId, relatedType: "blocker", relatedId: blocker.id, text: `Погодити блокер: ${blocker.title}. Факти: ${blocker.facts}. Реакція до ${dateLabel(blocker.decisionDue)}.${blocker.recommendation ? ` Рекомендація: ${blocker.recommendation}` : ""}`, kind: "approval", createdAt: isoNow() }); const target = state.nodes.find((node) => node.id === selected.id)!; target.health = "blocked"; target.decisionRequired = true; }); if (ok) setModal(null); }} />}
      ...
      {modal === "coordination" && selected?.kind === "cycle" && <CoordinationModal node={selected} payload={payload} currentUserId={payload.currentUser.id} notify={setNotice} close={() => setModal(null)} save={async (snapshot) => { const ok = await mutate(`Зафіксовано координацію напряму зусиль ${selected.code}`, selected.id, (state) => state.coordinations.unshift(snapshot)); if (ok) setModal(null); }} />}
...
function DashboardView({ data, payload, userById, healthFilter, setHealthFilter, select, openWork }: { data: ComputedData; payload: PortalPayload; userById: (id: string) => PortalUser | undefined; healthFilter: "all" | HealthStatus | "overdue" | "decision"; setHealthFilter: (value: "all" | HealthStatus | "overdue" | "decision") => void; select: (id: string) => void; openWork: (id: string) => void }) {
  ...
  return <>
        ...
        <div className="goal-list">
          {data.goals.map((goal) => {
            ...
            return <button key={goal.id} className="goal-card" onClick={() => select(goal.id)}>
              <div className="goal-code">{goal.code}</div><div className="goal-copy"><span>{goal.title}</span><h3>{goal.result}</h3><div className="goal-meta"><UserAvatar user={userById(goal.acceptorId)} compact /><small>{userById(goal.acceptorId)?.name}</small><i /> <small>{branchTasks.length} завдань</small></div></div><ProgressRing value={goal.progress} />
            </button>;
          })}
    ...
    <section className="panel cadence-strip">
      <div><span>Найближча координація напряму зусиль</span><strong>{coordination ? `${coordination.code} · ${coordination.title}` : "Напрямів зусиль ще немає"}</strong><small>{coordination ? "Предмет координації — зведений стан усіх завдань напряму зусиль з відображенням проектів." : "Після створення напряму зусиль тут з’явиться його зведений стан."}</small></div>
      ...
      <button disabled={!coordination} onClick={() => coordination && select(coordination.id)}>Перейти до напряму зусиль</button>
    </section>
...
function CalendarView({ payload, userById, openNode }: { payload: PortalPayload; userById: (id: string) => PortalUser | undefined; openNode: (id: string) => void }) {
  ...
  return <><PageIntro kicker="Часовий контур" title="Календар строків і координацій" text="Планові дати, прогнози, управлінські рішення, реакція на блокери та повторювані координації напрямів зусиль в одному місці." actions={<div className="calendar-header-actions"><div className="calendar-view-switch">{(["week", "month", "year"] as const).map((mode) => <button key={mode} className={viewMode === mode ? "active" : ""} onClick={() => setViewMode(mode)}>{mode === "week" ? "Тиждень" : mode === "month" ? "Місяць" : "Рік"}</button>)}</div><div className="calendar-period"><button onClick={() => changePeriod(-1)} aria-label="Попередній період">‹</button><button onClick={() => setAnchor(todayKey)}>Сьогодні</button><strong>{periodLabel}</strong><button onClick={() => changePeriod(1)} aria-label="Наступний період">›</button></div></div>} />
...
function TreeView(props: {
  ...
  openWork: (node: WorkNode) => void; mutate: (action: string, entityId: string, recipe: (state: PortalState) => void) => Promise<boolean>; copyNodeLink: (node: WorkNode, targetView: View) => Promise<void>; notify: Notify;
}) {
  const { payload, selected, selectedId, setSelectedId, search, setSearch, userById, canManage, locks, openCreateKind, openEdit, openWork, mutate, copyNodeLink, notify } = props;
  ...
  const [stateFilter, setStateFilter] = useState<"open" | "all" | "active" | "risk" | "completed" | "no_report_5d">("open");
  ...
  const matchesFilters = (node: WorkNode) => {
    ...
    const matchesOwner = ownerFilter === "all" || node.acceptorId === ownerFilter || node.assigneeId === ownerFilter;
  ...
  const startTreeResize = (event: React.PointerEvent<HTMLButtonElement>) => {
  ...
  const renderBranch = (parentId: string | null, depth = 0): React.ReactNode => {
    ...
    const mayEdit = canManage || node.acceptorId === payload.currentUser.id;
  ...
  const renderFlatResult = (node: WorkNode) => {
    ...
    return <div className="tree-filter-result" key={node.id}>
      <nav className="tree-filter-path" aria-label={`Шлях до ${node.code}`}>{path.length ? path.map((ancestor, index) => <span key={ancestor.id}><a href={portalHref("tree", ancestor.id)} title={`${kindLabels[ancestor.kind]} · ${ancestor.code} · ${ancestor.title}`} onClick={(event) => { if (!ordinaryLinkClick(event)) return; event.preventDefault(); openTreeCard(ancestor.id); }}>{ancestor.code}</a>{index < path.length - 1 && <i>/</i>}</span>) : <small>Верхній рівень</small>}</nav>
      <div className={`tree-row tree-flat-row ${node.id === selectedId ? "selected" : ""} kind-${node.kind}`}><span className="tree-flat-marker">·</span><a className="tree-row-main" href={portalHref("tree", node.id)} onClick={(event) => { if (!ordinaryLinkClick(event)) return; event.preventDefault(); openTreeCard(node.id); }} title={`${kindLabels[node.kind]} · ${node.code} · ${node.title}`}><span className="tree-code">{node.code}</span><span className="tree-name">{node.title}</span>{lock && <span className="editing-badge" title={`${lock.userName} редагує картку`}>✎ {lock.userName}</span>}<StatusBadge node={node} /></a>{treeMode === "archive" && mayEdit && <button className="flat-restore" onClick={() => void restoreBranch(node)} aria-label={`Відновити ${node.code}`}>↺</button>}</div>
    </div>;
  };

  return <>
    <PageIntro kicker="Структура управління" title="Дерево цілей, напрямів зусиль, проектів і завдань" text="Тут створюється структура та зберігаються паспорти. Виконання й звіти ведуться у «Моїй роботі»." actions={canManage && treeMode === "active" && <details className="create-uo-menu"><summary>+ Створити</summary><div>{(["goal", "cycle", "subcycle", "task"] as NodeKind[]).map((kind) => <button key={kind} onClick={(event) => { event.currentTarget.closest("details")?.removeAttribute("open"); openCreateKind(kind, selected); }}><span>{kind === "goal" ? "S" : kind === "cycle" ? "P" : kind === "subcycle" ? "P.x" : "✓"}</span>{kindLabels[kind]}</button>)}</div></details>} />
    ...
    <div className={`tree-workbench ${treeCompact ? "compact-tree" : ""} mobile-pane-${mobilePane}`} style={{ "--tree-nav-width": `${treeWidth}px` } as React.CSSProperties}>
      <aside className="tree-catalog">
        ...
        {!treeCompact && <div className="compact-filters"><select value={kindFilter} onChange={(event) => setKindFilter(event.target.value as "all" | NodeKind)}><option value="all">Усі рівні</option><option value="goal">Цілі</option><option value="cycle">Напрями зусиль</option><option value="subcycle">Проекти</option><option value="task">Завдання</option></select><select value={ownerFilter} onChange={(event) => setOwnerFilter(event.target.value)}><option value="all">Усі відповідальні</option>{payload.users.filter((user) => user.active).map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select><select value={stateFilter} onChange={(event) => setStateFilter(event.target.value as typeof stateFilter)}><option value="open">Усі без завершених</option><option value="no_report_5d">Немає звіту понад 5 днів</option><option value="all">Усі стани</option><option value="active">Активні</option><option value="risk">Ризик / блокер</option><option value="completed">Завершені</option></select></div>}
      ...
      {selected ? <section key={selected.id} className="node-detail">
        <header className="node-detail-head">
          <div><span>{kindLabels[selected.kind]} · {selected.code}</span><h2>{selected.title}</h2></div>
          <div className="node-head-actions"><ProgressRing value={selected.progress} /><button className="secondary" onClick={() => void copyNodeLink(selected, "tree")}>Копіювати посилання</button>{selected.archived ? <>{(canManage || selected.acceptorId === payload.currentUser.id) && <button className="restore-action" onClick={() => void restoreBranch(selected)}>Відновити з архіву</button>}<span className="archive-state">В архіві</span></> : hasWorkAccessForUser(selected, payload.currentUser) ? <button className="primary work-link" onClick={() => openWork(selected)}>Відкрити робочу картку →</button> : <span className="archive-state">Лише перегляд</span>}</div>
        </header>
        <div className="status-line"><StatusBadge node={selected} />{selected.decisionRequired && <span className="decision-badge">Потрібне рішення</span>}<span>{lifecycleLabels[selected.lifecycle]}</span><CreationNote node={selected} payload={payload} /><span>Оновлено {new Date(selected.updatedAt).toLocaleString("uk-UA")}</span></div>
        {selected.healthComment && <div className={`health-comment-banner ${selected.health}`}><strong>{selected.health === "blocked" ? "Блокер" : "Ризик"}</strong><p>{selected.healthComment}</p></div>}
        ...
        {detailTab === "passport" && <WorkCardDescription node={selected} payload={payload} userById={userById} expanded />}
        {detailTab === "passport" && <TreeWorkSnapshot node={selected} payload={payload} userById={userById} />}
        {detailTab === "passport" && selected.kind === "subcycle" && <ProjectTaskResults node={selected} payload={payload} userById={userById} />}
        {detailTab === "passport" && !selected.archived && <DiscussionPanel key={selected.id} node={selected} payload={payload} mutate={mutate} notify={notify} userById={userById} />}
        {detailTab === "passport" && selected.evidence.length > 0 && <TreeEvidence node={selected} />}
...
function NotificationsView({ payload, userById, mutate, openNode }: { payload: PortalPayload; userById: (id: string) => PortalUser | undefined; mutate: (action: string, entityId: string, recipe: (state: PortalState) => void) => Promise<boolean>; openNode: (id: string, filter: WorkFilter, focus: WorkFocus) => void }) {
  ...
  const routeFor = (nodeId: string): WorkFilter => workFilterForUser(payload.nodes.find((item) => item.id === nodeId), payload.currentUser);
...
function MyWork({ payload, selected, selectedId, setSelectedId, initialFilter, focusTarget, userById, canManage, saveWorkUpdate, resolveAcceptance, completeNode, openEdit, copyNodeLink, setModal, asanaStatus, mutate, setNotice, openTree }: { payload: PortalPayload; selected?: WorkNode; selectedId: string; setSelectedId: (id: string) => void; initialFilter: WorkFilter; focusTarget: WorkFocus; userById: (id: string) => PortalUser | undefined; canManage: boolean; saveWorkUpdate: (node: WorkNode, update: Omit<WorkUpdate, "id" | "createdAt" | "createdBy" | "source">) => Promise<boolean>; resolveAcceptance: (acceptance: Acceptance, accepted: boolean, feedback?: string) => Promise<boolean>; completeNode: (node: WorkNode) => Promise<boolean>; openEdit: (node: WorkNode) => Promise<void>; copyNodeLink: (node: WorkNode, targetView: View) => Promise<void>; setModal: (modal: Modal) => void; asanaStatus: { configured: boolean; connected: boolean; connection?: Record<string, string> } | null; mutate: (action: string, entityId: string, recipe: (state: PortalState) => void) => Promise<boolean>; setNotice: Notify; openTree: (id: string) => void }) {
  ...
  const [sort, setSort] = useState<"code" | "deadline" | "priority" | "updated" | "progress">("code");
  ...
  const [workHealthFilter, setWorkHealthFilter] = useState<"all" | HealthStatus | "branch_blocker" | "no_report_5d">("all");
  ...
  const mine = payload.nodes.filter((node) => !node.archived && (node.assigneeId === payload.currentUser.id || node.acceptorId === payload.currentUser.id || node.participantIds.includes(payload.currentUser.id) || incomingNodeIds.has(node.id)));
  const priorityOrder = { critical: 0, high: 1, normal: 2, low: 3 };
  const filtered = mine.filter((node) => filter === "all" || filter === "action" && node.assigneeId === payload.currentUser.id && !["idea", "completed", "cancelled"].includes(node.lifecycle) || filter === "acceptance" && node.acceptorId === payload.currentUser.id)
    ...
    .filter((node) => workHealthFilter === "all" || workHealthFilter === "branch_blocker" && branchHasOpenBlocker(payload, node) || workHealthFilter === "no_report_5d" && lacksRecentReport(node) || node.health === workHealthFilter)
    .filter((node) => priorityFilter === "all" || node.priority === priorityFilter)
    .sort((a, b) => sort === "code" ? compareNodeCodes(a, b) : sort === "deadline" ? (a.plannedEnd || "9999").localeCompare(b.plannedEnd || "9999") || compareNodeCodes(a, b) : sort === "priority" ? priorityOrder[a.priority] - priorityOrder[b.priority] || compareNodeCodes(a, b) : sort === "progress" ? a.progress - b.progress || compareNodeCodes(a, b) : b.updatedAt.localeCompare(a.updatedAt) || compareNodeCodes(a, b));
  ...
  useEffect(() => {
    ...
    window.requestAnimationFrame(() => document.getElementById(focusTarget === "discussion" ? `discussion-${current.id}` : focusTarget === "reports" ? `reports-${current.id}` : `manager-actions-${current.id}`)?.scrollIntoView({ behavior: "smooth", block: "start" }));
  }, [current, focusTarget]);
  ...
  const mayWork = current && (canManage || current.acceptorId === payload.currentUser.id || current.assigneeId === payload.currentUser.id);
  const mayEditCard = current && (canManage || current.acceptorId === payload.currentUser.id);
  ...
  return <>
    <div className="my-workbench">
      <aside className="work-inbox">
        <div className="work-filter">{(["action", "acceptance", "all"] as const).map((item) => <button key={item} className={filter === item ? "active" : ""} onClick={() => setFilter(item)}>{item === "action" ? "Мої дії" : item === "acceptance" ? "Приймаю" : "Усі"}</button>)}</div>
        <div className="work-list-controls"><input value={listQuery} onChange={(event) => setListQuery(event.target.value)} placeholder="Пошук…" /><select value={sort} onChange={(event) => setSort(event.target.value as typeof sort)}><option value="code">За номером</option><option value="deadline">За строком</option><option value="priority">За пріоритетом</option><option value="updated">За оновленням</option><option value="progress">За прогресом</option></select></div>
        <details className="work-advanced-filters"><summary><span>Рівень, статус і стан</span>{activeFilterCount > 0 && <b>{activeFilterCount}</b>}<i>⌄</i></summary><div><label><span>Рівень</span><select value={levelFilter} onChange={(event) => setLevelFilter(event.target.value as typeof levelFilter)}><option value="all">Усі рівні</option><option value="goal">Цілі</option><option value="cycle">Напрями зусиль</option><option value="subcycle">Проекти</option><option value="task">Завдання</option></select></label><label><span>Статус</span><select value={lifecycleFilter} onChange={(event) => setLifecycleFilter(event.target.value as typeof lifecycleFilter)}><option value="all">Усі статуси</option>{Object.entries(lifecycleLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label><span>Стан</span><select value={workHealthFilter} onChange={(event) => setWorkHealthFilter(event.target.value as typeof workHealthFilter)}><option value="all">Будь-який стан</option><option value="branch_blocker">Блокери</option><option value="no_report_5d">Немає звіту понад 5 днів</option><option value="normal">Нормально</option><option value="risk">Є ризик</option><option value="blocked">Заблоковано</option></select></label><label><span>Пріоритет</span><select value={priorityFilter} onChange={(event) => setPriorityFilter(event.target.value as typeof priorityFilter)}><option value="all">Усі пріоритети</option><option value="critical">Критичний</option><option value="high">Високий</option><option value="normal">Нормальний</option><option value="low">Низький</option></select></label><button type="button" disabled={!activeFilterCount} onClick={resetListFilters}>Скинути фільтри</button></div></details>
        <div className="work-inbox-list">{filtered.map((node) => <a className={`work-list-row kind-${node.kind} ${node.id === current?.id ? "selected" : ""}`} key={node.id} href={portalHref("my", node.id)} onClick={(event) => { if (!ordinaryLinkClick(event)) return; event.preventDefault(); setSelectedId(node.id); }}><div><span>{kindLabels[node.kind]} · {node.code}</span><strong>{node.title}</strong></div><StatusBadge node={node} /><footer><small>{node.progress}%</small><time>{dateLabel(node.plannedEnd)}</time></footer></a>)}{!filtered.length && <p className="empty-state padded">За вибраними фільтрами карток немає.</p>}</div>
      </aside>
      {current ? <section className="work-desk"><header className="work-desk-head"><div><span>{kindLabels[current.kind]} · {current.code}</span><h2>{current.title}</h2><p>{current.result}</p></div><div className="work-desk-actions"><ProgressRing value={current.progress} />{!["acceptance", "completed"].includes(current.lifecycle) && mayWork && <button className="positive primary-completion" disabled={Boolean(completionReason)} title={completionReason || "Передати результат на приймання"} onClick={() => void completeNode(current)}>Завершити</button>}<details className="work-card-menu"><summary aria-label="Інші дії з карткою">⋮ Дії</summary><div>{mayEditCard && <button onClick={(event) => { event.currentTarget.closest("details")?.removeAttribute("open"); void openEdit(current); }}>Редагувати картку</button>}<button onClick={(event) => { event.currentTarget.closest("details")?.removeAttribute("open"); void copyNodeLink(current, "my"); }}>Копіювати посилання</button>{current.kind === "task" && <button onClick={(event) => { event.currentTarget.closest("details")?.removeAttribute("open"); scrollToSection(`asana-link-${current.id}`); }}>Перейти до Asana</button>}<button onClick={(event) => { event.currentTarget.closest("details")?.removeAttribute("open"); openTree(current.id); }}>Паспорт у дереві</button></div></details></div></header><div className="status-line"><StatusBadge node={current} />{current.decisionRequired && <span className="decision-badge">Потрібне рішення</span>}<span>Строк {dateLabel(current.plannedEnd)}</span><span>Прогноз {dateLabel(current.forecastEnd)}</span><CreationNote node={current} payload={payload} />{completionReason && <span className="completion-hint">{completionReason}</span>}</div><nav className="work-card-sections" aria-label="Розділи робочої картки"><button onClick={() => scrollToSection(`card-description-${current.id}`)}>Опис</button>{current.kind === "task" && <><button onClick={() => scrollToSection(`work-status-${current.id}`)}>Стан і звіт</button><button onClick={() => scrollToSection(`evidence-${current.id}`)}>Докази</button><button onClick={() => scrollToSection(`asana-link-${current.id}`)}>Asana</button><button onClick={() => scrollToSection(`work-history-${current.id}`)}>Історія</button></>}<button onClick={() => scrollToSection(`discussion-${current.id}`)}>Комунікація</button><button onClick={() => scrollToSection(`work-audit-${current.id}`)}>Журнал змін</button></nav>
        <div className="work-summary-grid"><article><span>Виконавець</span><div className="person-line"><UserAvatar user={userById(current.assigneeId)} compact /><strong>{userById(current.assigneeId)?.name}</strong></div></article><article><span>Ініціатор</span><div className="person-line"><UserAvatar user={userById(current.acceptorId)} compact /><strong>{userById(current.acceptorId)?.name}</strong></div></article></div>
        <WorkCardDescription node={current} payload={payload} userById={userById} />
        {current.kind === "task" ? <div className="execution-grid"><div><WorkStatusForm key={current.id} node={current} currentUserId={payload.currentUser.id} disabled={!mayWork} notify={setNotice} save={(update) => saveWorkUpdate(current, update)} /><section id={`evidence-${current.id}`} className="work-section section-anchor"><div className="work-section-head"><div><span>Результат</span><h3>Докази та передання</h3></div>{mayWork && <button onClick={() => setModal("evidence")}>+ Додати доказ</button>}</div><p>{current.acceptanceCriteria}</p><div className="evidence-stack">{current.evidence.map((item) => <a key={item.id} href={item.kind === "note" ? undefined : item.value} target="_blank" rel="noreferrer"><b>{item.label}</b><span>{item.kind === "file" ? "Файл" : item.kind === "link" ? "Посилання" : item.value}</span></a>)}{!current.evidence.length && <span className="empty-state">Доказів ще немає.</span>}</div>{mayWork && <div className="work-action-row"><button onClick={() => setModal("blocker")}>Додати блокер</button><button onClick={() => setModal("decision")}>Запитати рішення</button><button onClick={() => setModal("dependency")}>Додати залежність</button></div>}<small className="completion-explainer">Після натискання «Завершити» результат автоматично надсилається ініціатору. Статус «Завершено» і 100% встановлюються після приймання ініціатором.</small></section></div><aside><AsanaSyncPanel key={`${current.id}-${current.asana.taskGid}`} payload={payload} selected={current} asanaStatus={asanaStatus} mutate={mutate} setNotice={setNotice} compact /><WorkUpdateHistory node={current} userById={userById} /></aside></div> : <div className="aggregate-work"><section className="work-section aggregate-note"><span>Агрегований рівень</span><h3>Стан цього рівня формується з нижчих рівнів</h3><p>{current.kind === "cycle" ? "Зведений стан напряму зусиль формується з усіх його проектів і завдань та є предметом координації." : "Проект показує агрегований стан своїх завдань і входить до координації батьківського напряму зусиль."}</p>{current.kind === "cycle" && <button className="primary" onClick={() => setModal("coordination")}>Зафіксувати координацію напряму зусиль</button>}</section><section className="work-section"><div className="work-section-head"><div><span>Нижчі рівні</span><h3>Джерела стану</h3></div><b>{children.length}</b></div><div className="aggregate-children">{children.map((node) => <button key={node.id} onClick={() => setSelectedId(node.id)}><span>{node.code}</span><strong>{node.title}</strong><em>{node.progress}%</em><StatusBadge node={node} /></button>)}</div></section></div>}
        {current.kind === "subcycle" && <ProjectTaskResults node={current} payload={payload} userById={userById} />}
        ...
        <NodeAuditHistory node={current} audit={payload.audit} />
      </section> : <section className="work-desk empty-work"><h2>{mine.length ? "За вибраним фільтром карток немає" : "Робочих карток ще немає"}</h2><p>{mine.length ? "Змініть режим або додаткові фільтри, щоб побачити інші об’єкти." : "Після створення цілі, напряму зусиль або завдання доступні вам картки з’являться тут."}</p></section>}
    </div>
  </>;
}
...
function WorkStatusForm({ node, currentUserId, disabled, save, notify }: { node: WorkNode; currentUserId: string; disabled: boolean; save: (update: Omit<WorkUpdate, "id" | "createdAt" | "createdBy" | "source">) => Promise<boolean>; notify: Notify }) {
  ...
  const submit = async () => {
    if (node.kind === "task" && form.lifecycle === "completed") {
      ...
      notify("Для завершення завдання використайте кнопку «Завершити»: результат має пройти приймання ініціатором.", "error");
      return;
    }
  ...
  const forecastMovesDeadline = Boolean(form.forecastEnd && (!node.plannedEnd || form.forecastEnd > node.plannedEnd));
  return <section className="work-section status-update"><div className="work-section-head"><div><span>Фактичний стан</span><h3>Оновлення та звіт</h3></div><StatusBadge node={{ ...node, lifecycle: form.lifecycle, health: form.health }} /></div><div className="status-form-grid"><Field label="Стан" required hint={node.kind === "task" ? "Оновіть фактичний етап. Для завершення використайте кнопку «Завершити»: результат перейде на приймання ініціатору." : "Фактичний етап виконання на момент звіту."}><select value={form.lifecycle} disabled={disabled} onChange={(event) => setForm({ ...form, lifecycle: event.target.value as LifecycleStatus })}>{lifecycleOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field><Field label="Стан здоров’я" required hint="Нормально — рух за планом; ризик — строк або результат під загрозою; заблоковано — рух неможливий без окремої реакції."><select value={form.health} disabled={disabled} onChange={(event) => setForm({ ...form, health: event.target.value as HealthStatus })}>{Object.entries(healthLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field><Field label={`Прогрес · ${form.progress}%`} required hint="Оцініть частку вже отриманого результату, а не витрачений час."><input type="range" min="0" max="100" value={form.progress} disabled={disabled} onChange={(event) => setForm({ ...form, progress: Number(event.target.value) })} /></Field><Field label="Прогноз завершення" hint="Якщо прогноз пізніше строку завершення картки, після збереження звіту цей строк автоматично перенесеться на дату прогнозу. Раніший прогноз не скорочує строк."><input type="date" value={form.forecastEnd} disabled={disabled} onChange={(event) => setForm({ ...form, forecastEnd: event.target.value })} />{forecastMovesDeadline && <small className="forecast-deadline-notice">Після збереження строк картки: {dateLabel(form.forecastEnd)}</small>}</Field><Field wide label="Що зроблено / фактичний результат" required error={summaryError} hint="Фіксуйте перевірюваний результат, а не процес або витрачений час." example="Оновлено сторінку послуги, перевірено 12 посилань, передано на погодження власнику результату."><textarea value={form.summary} disabled={disabled} aria-invalid={Boolean(summaryError)} onChange={(event) => { setForm({ ...form, summary: event.target.value }); setSummaryError(""); }} placeholder="Наприклад: підготовлено макет і передано на погодження" /></Field><Field wide label="Наступна дія" hint="Наступний конкретний крок, відповідальний та умова або строк." example="До 18.08 Валентина вносить правки після погодження Володимира."><textarea value={form.nextAction} disabled={disabled} onChange={(event) => setForm({ ...form, nextAction: event.target.value })} placeholder="Наприклад: внести правки після погодження власника" /></Field></div><button className="primary full-action" disabled={disabled || busy} onClick={() => void submit()}>{busy ? "Зберігаємо…" : "Зберегти стан і подати звіт"}</button></section>;
}

function WorkUpdateHistory({ node, userById }: { node: WorkNode; userById: (id: string) => PortalUser | undefined }) {
  return <section id={`reports-${node.id}`} className="work-section update-panel section-anchor"><div className="work-section-head"><div><span>Системний слід</span><h3>Останні звіти</h3></div><b>{(node.updates || []).length}</b></div><div className="update-history">{(node.updates || []).slice(0, 8).map((update) => <article key={update.id}><time>{new Date(update.createdAt).toLocaleString("uk-UA")}</time><strong>{lifecycleLabels[update.lifecycle]} · {update.progress}%</strong><p>{update.summary}</p><small>{userById(update.createdBy)?.name || update.externalAuthorName || "Asana"}{update.nextAction ? ` · Далі: ${update.nextAction}` : ""}</small></article>)}{!(node.updates || []).length && <p className="empty-state">Звітів ще немає.</p>}</div></section>;
}

function AuditEntryDetails({ entry }: { entry: AuditEntry }) {
  return <><strong>{entry.action}</strong><small>{entry.by}</small>{Boolean(entry.changes?.length) && <div className="audit-changes">{entry.changes!.map((change) => <div key={change.field}><b>{change.label}</b><span><del>{change.from}</del><span aria-hidden="true">→</span><ins>{change.to}</ins></span></div>)}</div>}</>;
}

function NodeAuditHistory({ node, audit }: { node: WorkNode; audit: AuditEntry[] }) {
  const entries = audit.filter((entry) => entry.entityId === node.id);
  return <section id={`work-audit-${node.id}`} className="work-section node-audit-history section-anchor"><div className="work-section-head"><div><span>Серверний журнал</span><h3>Хто й що змінив</h3></div><b>{entries.length}</b></div><div className="update-history">{entries.map((entry) => <article key={entry.id}><time>{new Date(entry.at).toLocaleString("uk-UA")}</time><AuditEntryDetails entry={entry} /></article>)}{!entries.length && <p className="empty-state">Записів змін для цієї картки ще немає.</p>}</div></section>;
}

function ProjectTaskResults({ node, payload, userById, compact = false }: { node: WorkNode; payload: PortalPayload; userById: (id: string) => PortalUser | undefined; compact?: boolean }) {
  const tasks = descendants(payload.nodes, node.id).filter((item) => item.kind === "task" && !item.archived).sort(compareNodeCodes);
  const reported = projectTaskResults(payload.nodes, node.id);
  const reportedTaskCount = new Set(reported.map(({ task }) => task.id)).size;
  return <section className={`project-task-results ${compact ? "compact" : "work-section"}`} aria-label={`Результати завдань проекту ${node.code}`}><div className="work-section-head"><div><span>Зведено із завдань · {reportedTaskCount} із {tasks.length} мають звіти · усього {reported.length} звітів</span><h3>Результати й наступні дії проєкту</h3></div></div><div className="project-results-list">{reported.map(({ task, report }) => <article key={report.id}><a href={cardHref(task, payload.currentUser)} title={`Відкрити завдання ${task.code}`}><b>{task.code}</b> {task.title}</a><small>{new Date(report.createdAt).toLocaleString("uk-UA")} · {userById(report.createdBy)?.name || (report.source === "asana" ? "Asana" : "Учасник")}</small><p><strong>Результат:</strong> {report.summary}</p><p><strong>Наступна дія:</strong> {report.nextAction || "Не вказана"}</p></article>)}{!reported.length && <p className="empty-state">У завданнях цього проєкту ще немає звітів.</p>}</div></section>;
}

function WorkCardDescription({ node, payload, userById, expanded = false }: { node: WorkNode; payload: PortalPayload; userById: (id: string) => PortalUser | undefined; expanded?: boolean }) {
  const titles = useAsanaTitles(node);
  const path = nodePath(payload.nodes, node);
  const participants = node.participantIds.map((id) => userById(id)?.name).filter(Boolean).join(", ");
  const value = (text: string) => <LinkedText value={text.trim() || "Не визначено"} asanaTitles={titles} />;
  const body = <div className="work-card-description-body"><section><h4>Місце в структурі</h4><dl><dt>Шлях</dt><dd>{path.map((item) => `${item.code} · ${item.title}`).join(" → ")}</dd><dt>Рівень</dt><dd>{kindLabels[node.kind]}</dd><dt>Код і назва</dt><dd>{node.code} · {node.title}</dd></dl></section><section><h4>Результат і межі</h4><dl><dt>Опис</dt><dd>{value(node.description)}</dd><dt>Готовий результат</dt><dd>{value(node.result)}</dd><dt>Що не є результатом</dt><dd>{value(node.nonResult)}</dd><dt>Критерій приймання</dt><dd>{value(node.acceptanceCriteria)}</dd></dl></section><section><h4>Відповідальні</h4><dl><dt>Виконавець</dt><dd>{userById(node.assigneeId)?.name || "Не визначено"}</dd><dt>Ініціатор</dt><dd>{userById(node.acceptorId)?.name || "Не визначено"}</dd><dt>Учасники / фоловери</dt><dd>{participants || "Не визначено"}</dd></dl></section><section><h4>Строки та стан</h4><dl><dt>Плановий початок</dt><dd>{dateLabel(node.plannedStart)}</dd>{showActualMilestones(node.kind) && <><dt>Фактичний початок</dt><dd>{dateLabel(node.actualStart)}</dd></>}<dt>Дедлайн до</dt><dd>{dateLabel(node.plannedEnd)}</dd><dt>Прогноз завершення</dt><dd>{dateLabel(node.forecastEnd)}</dd>{showActualMilestones(node.kind) && <><dt>Фактичне завершення</dt><dd>{dateLabel(node.actualEnd)}</dd></>}<dt>Статус</dt><dd>{lifecycleLabels[node.lifecycle]} · {node.progress}%</dd><dt>Стан виконання</dt><dd>{healthLabels[node.health]}</dd>{node.healthComment && <><dt>Коментар до стану</dt><dd>{node.healthComment}</dd></>}<dt>Пріоритет</dt><dd>{priorityLabels[node.priority]}</dd><dt>Потрібне рішення</dt><dd>{node.decisionRequired ? "Так" : "Ні"}</dd></dl></section><section><h4>Умови виконання</h4><dl><dt>Спосіб початку</dt><dd>{startLabels[node.startMode]}</dd><dt>Обмеження повноважень</dt><dd>{value(node.authority)}</dd><dt>Ресурс</dt><dd>{value(node.resource)}</dd><dt>Контрольне місце</dt><dd>{value(node.controlPlace)}</dd><dt>Доступ</dt><dd>{"Усі активні користувачі порталу (перегляд)"}</dd><dt>Повторення</dt><dd>{node.recurrence?.enabled ? ({ weekly: "Щотижня", monthly: "Щомісяця", quarterly: "Щокварталу", yearly: "Щороку" }[node.recurrence.frequency]) : "Немає"}</dd>{node.kind === "cycle" && <><dt>Координація</dt><dd>{coordinationCadenceLabel(node)}</dd><dt>Перша координація</dt><dd>{dateLabel(node.coordinationStartDate || "")}</dd></>}</dl></section></div>;
  if (expanded) return <section className="work-card-description tree-passport"><header className="tree-passport-heading"><span>Паспорт робочої картки</span><h3>Опис картки</h3></header>{body}</section>;
  return <details className="work-card-description"><summary><div><span>Паспорт робочої картки</span><strong>Опис картки</strong><small>{path.map((item) => item.code).join(" / ")}</small></div><i className="details-state" aria-hidden="true" /></summary>{body}</details>;
}
...
function OpenBlockersPanel({ node, payload, userById, mutate }: { node: WorkNode; payload: PortalPayload; userById: (id: string) => PortalUser | undefined; mutate: (action: string, entityId: string, recipe: (state: PortalState) => void) => Promise<boolean> }) {
  ...
  return <section id={`blockers-${node.id}`} className="work-section open-blockers-panel section-anchor"><div className="work-section-head"><div><span>Контроль відхилень</span><h3>Відкриті блокери</h3></div><b>{blockers.length}</b></div><div className="open-blocker-list">{blockers.map((blocker) => { const mayResolve = administrator || blocker.ownerId === payload.currentUser.id || node.acceptorId === payload.currentUser.id || node.assigneeId === payload.currentUser.id; return <article key={blocker.id}><header><div><span>{blocker.approvalStatus === "approved" ? "Підтверджено" : "Очікує погодження"}</span><strong>{blocker.title}</strong></div><time>реакція до {dateLabel(blocker.decisionDue)}</time></header><p>{blocker.facts}</p><dl><dt>Відповідальний</dt><dd>{userById(blocker.ownerId)?.name || "Не визначено"}</dd><dt>Ескалація</dt><dd>{userById(blocker.escalationToId)?.name || "Не визначено"}</dd>{blocker.approvalComment && <><dt>Коментар погодження</dt><dd>{blocker.approvalComment}</dd></>}</dl>{mayResolve ? <div className="blocker-resolution"><textarea value={comments[blocker.id] || ""} onChange={(event) => setComments((current) => ({ ...current, [blocker.id]: event.target.value }))} placeholder="Що зроблено і чому блокер більше не зупиняє роботу…" /><button disabled={busyId === blocker.id || !comments[blocker.id]?.trim()} onClick={() => void resolve(blocker)}>Позначити усуненим</button></div> : <small>Усунення фіксує відповідальний за блокер, виконавець або ініціатор картки.</small>}</article>; })}</div></section>;
}
...
function DiscussionPanel({ node, payload, mutate, notify, userById }: { node: WorkNode; payload: PortalPayload; mutate: (action: string, entityId: string, recipe: (state: PortalState) => void) => Promise<boolean>; notify: Notify; userById: (id: string) => PortalUser | undefined }) {
  const [draft, setDraft, clearDraft] = usePersistentDraft(`portal:discussion-draft:${payload.currentUser.id}:${node.id}`, { text: "", kind: "comment" as "comment" | "question" | "issue", recipientId: node.acceptorId, requiresResponse: false });
  const [busy, setBusy] = useState(false);
  ...
  const effectiveRecipientId = draft.kind === "issue" ? node.assigneeId : draft.recipientId || node.acceptorId;
  const send = async () => {
    if (!draft.text.trim()) { notify("Напишіть повідомлення для учасників картки.", "error"); return; }
    if ((draft.kind === "question" || draft.kind === "issue" || draft.requiresResponse) && !effectiveRecipientId) { notify("Оберіть адресата повідомлення.", "error"); return; }
    setBusy(true);
    const message: DiscussionMessage = { id: crypto.randomUUID(), nodeId: node.id, authorId: payload.currentUser.id, recipientId: replyTo?.authorId || (draft.kind === "question" || draft.kind === "issue" || draft.requiresResponse ? effectiveRecipientId : undefined), replyToId: replyTo?.id, requiresResponse: !replyTo && (draft.kind === "question" || draft.kind === "issue" || draft.requiresResponse), text: draft.text.trim(), kind: draft.kind, createdAt: isoNow() };
    const ok = await mutate(`${replyTo ? "Додано відповідь" : draft.kind === "issue" ? "Повідомлено про перешкоду" : draft.kind === "question" ? "Поставлено питання" : "Додано коментар"} до ${node.code}`, node.id, (state) => {
    ...
    if (ok) { setDraft({ text: "", kind: "comment", recipientId: node.acceptorId, requiresResponse: false }); setReplyToId(""); clearDraft(); }
    setBusy(false);
  };
  ...
  const mayChangeMessage = (message: DiscussionMessage) => !message.asanaStoryGid && ["comment", "question", "issue"].includes(message.kind) && (message.authorId === payload.currentUser.id || node.acceptorId === payload.currentUser.id || ["owner", "admin"].includes(payload.currentUser.role));
  ...
  return <section id={`discussion-${node.id}`} className="work-section discussion-panel">
    ...
    <div className="discussion-list">{messages.slice(-30).map((message) => {
      const awaitingResponse = !message.resolvedAt && (message.kind === "question" || message.kind === "issue" || message.requiresResponse);
      const canRespond = [message.authorId, message.recipientId, node.acceptorId].includes(payload.currentUser.id) || ["owner", "admin"].includes(payload.currentUser.role);
      const editing = editingMessageId === message.id;
      return <article key={message.id} className={`${message.kind} ${message.resolvedAt ? "resolved" : ""}`}><div><UserAvatar compact user={userById(message.authorId)} /><strong>{userById(message.authorId)?.name || message.externalAuthorName || "Система"}</strong>{message.recipientId && <span className="message-recipient">→ {userById(message.recipientId)?.name || "адресат"}</span>}{message.kind === "issue" && <span className="question-state">{message.resolvedAt ? "Перешкоду розглянуто" : "Сигнал про перешкоду · не блокер"}</span>}{message.kind === "question" && <span className="question-state">{message.resolvedAt ? "Відповідь отримано" : "Відкрите питання"}</span>}{message.kind === "comment" && message.requiresResponse && <span className="question-state">{message.resolvedAt ? "Відповідь отримано" : "Очікує відповіді"}</span>}{message.kind === "decision" && <span className="question-state">{message.resolvedAt ? "Рішення прийнято" : "Очікує рішення"}</span>}<time>{new Date(message.createdAt).toLocaleString("uk-UA")}{message.editedAt ? " · змінено" : ""}</time>{mayChangeMessage(message) && !editing && <div className="message-actions"><button onClick={() => { setEditingMessageId(message.id); setEditingText(message.text); }}>Редагувати</button><button className="delete" onClick={() => void deleteMessage(message)}>Видалити</button></div>}</div>{message.replyToId && <small className="reply-reference">Відповідь на попереднє повідомлення</small>}{editing ? <div className="message-edit-form"><textarea value={editingText} onChange={(event) => setEditingText(event.target.value)} /><div><button onClick={() => { setEditingMessageId(""); setEditingText(""); }}>Скасувати</button><button className="primary" onClick={() => void saveMessageEdit(message)}>Зберегти зміни</button></div></div> : <p>{message.text}</p>}{awaitingResponse && canRespond && !editing && <div className="question-actions"><button onClick={() => { setReplyToId(message.id); setDraft({ ...draft, kind: "comment", recipientId: message.authorId, requiresResponse: false }); }}>Відповісти</button><button className="resolve-question" onClick={() => void resolveQuestion(message)}>Закрити без відповіді</button></div>}</article>;
    })}{!messages.length && <p className="empty-state">Повідомлень ще немає. Коментарі й сигнали про перешкоди зберігатимуться в картці та у «Вхідних» адресатів.</p>}</div>
...
function CoordinationView({ payload, userById, open }: { payload: PortalPayload; userById: (id: string) => PortalUser | undefined; open: (node: WorkNode) => void }) {
  const [query, setQuery] = useState("");
  const [ownerId, setOwnerId] = useState("all");
  ...
  const renderCoordinationRow = (node: WorkNode, depth = 0): React.ReactNode => {
    ...
    return <div key={node.id} className={`coordination-tree-branch level-${node.kind}`}>
      <div className="coordination-tree-row" style={{ "--coord-depth": depth } as React.CSSProperties}>
        <div className="coordination-object-cell"><button className={`coordination-expand ${hasChildren ? "has-children" : "is-leaf"}`} type="button" disabled={!hasChildren} aria-expanded={hasChildren ? opened : undefined} title={hasChildren ? `${opened ? "Згорнути" : "Розгорнути"} ${node.code}` : undefined} onClick={() => toggleRow(node.id)} aria-label={hasChildren ? `${opened ? "Згорнути" : "Розгорнути"} ${node.code}` : `${node.code} без нижчого рівня`}>{hasChildren ? opened ? "⌄" : "›" : ""}</button><a className="coordination-object" href={cardHref(node, payload.currentUser)} target="_blank" rel="noopener noreferrer" title={`Відкрити ${node.code} у новій вкладці`}><span>{node.code}</span><strong>{node.title}</strong><small>{node.kind === "cycle" ? `${payload.nodes.find((item) => item.id === node.parentId)?.code || "Без цілі"} · ${coordinationCadenceLabel(node)}` : kindLabels[node.kind]}</small></a><button type="button" className="coordination-info-toggle" aria-label={`${openInfoIds.has(node.id) ? "Сховати" : "Показати"} межі результату ${node.code}`} aria-expanded={openInfoIds.has(node.id)} title="Опис, результат, межі та критерії приймання" onClick={() => setOpenInfoIds((current) => { const next = new Set(current); if (next.has(node.id)) next.delete(node.id); else next.add(node.id); return next; })}>і</button></div>
        <span className="coordination-owner">{userById(node.assigneeId)?.name || "—"}</span><StatusBadge node={node} /><span className="coordination-progress">{node.progress}%</span><time>{dateLabel(node.plannedEnd)}</time><div className="coordination-row-attention">{reasons.slice(0, 2).map((reason) => <span key={reason}>{reason}</span>)}{reasons.length > 2 && <b>+{reasons.length - 2}</b>}{node.kind === "cycle" && (["owner", "admin", "goal_owner", "cycle_owner"].includes(payload.currentUser.role) || node.acceptorId === payload.currentUser.id || node.assigneeId === payload.currentUser.id) && <button onClick={() => open(node)}>Координація</button>}</div>
      </div>
      {openInfoIds.has(node.id) && <div className="coordination-info-panel"><div className="coordination-info-grid">{([ ["Опис", node.description], ["Що є результатом", node.result], ["Що не є результатом", node.nonResult], ["Критерії прийняття", node.acceptanceCriteria] ] as [string, string][]).map(([label, value]) => <section key={label}><h4>{label}</h4><p>{value?.trim() || "Не визначено"}</p></section>)}</div>{node.kind === "subcycle" && <ProjectTaskResults node={node} payload={payload} userById={userById} compact />}</div>}
  ...
  return <><PageIntro kicker="Одиниця координації" title="Координація за напрямами зусиль" text="Предмет координації — зведений стан усіх завдань напряму зусиль з розрізом за проектами, строками, блокерами, рішеннями та прийманням." actions={<div className="page-filter-bar coordination-filter-bar"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Пошук цілі, напряму зусиль, проекту або завдання…" /><select value={level} onChange={(event) => setLevel(event.target.value as typeof level)}><option value="all">Усі рівні</option><option value="goal">Цілі</option><option value="cycle">Напрями зусиль</option><option value="subcycle">Проекти</option><option value="task">Завдання</option></select><select value={ownerId} onChange={(event) => setOwnerId(event.target.value)}><option value="all">Усі виконавці</option>{payload.users.filter((user) => user.active).map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select><CoordinationStateMultiSelect value={stateFilters} onChange={setStateFilters} /></div>} />
    <section className="panel coordination-attention"><div className="panel-head"><div><span>Контроль і реакція</span><h2>Потребує координації</h2></div><b className="count amber">{attentionNodes.length}</b></div><div className="coordination-attention-table"><div className="coordination-attention-head"><span>Об’єкт</span><span>Причини</span><span>Відповідальний</span><span>Строк</span></div>{attentionNodes.map(({ node, reasons }) => <a key={node.id} href={cardHref(node, payload.currentUser)} target="_blank" rel="noopener noreferrer" title={`Відкрити ${node.code} у новій вкладці`}><div><span>{node.code} · {kindLabels[node.kind]}</span><strong>{node.title}</strong></div><div>{reasons.map((reason) => <span key={reason}>{reason}</span>)}</div><span>{userById(node.assigneeId)?.name || "—"}</span><time>{dateLabel(node.plannedEnd)}</time></a>)}{!attentionNodes.length && <p className="empty-state padded">За вибраними фільтрами немає об’єктів, що потребують координації.</p>}</div></section>
    <section className="panel coordination-tree-table"><div className="panel-head"><div><span>Зведена звітність напрямів зусиль</span><h2>Стратегічна ціль → напрям зусиль → проект → завдання → три останні звіти</h2></div><div className="coordination-tree-controls"><button className={structureExpanded ? "active" : ""} onClick={toggleStructure}>{structureExpanded ? "Згорнути рівні" : "Розгорнути рівні"}</button><button className={reportsExpanded ? "active" : ""} disabled={!reportIds.length} onClick={toggleReports}>{reportsExpanded ? "Сховати звіти" : "Показати звіти"}</button><button className={everythingExpanded ? "active" : ""} disabled={!allExpandableIds.length} onClick={toggleEverything}>{everythingExpanded ? "Згорнути все" : "Розгорнути все"}</button><b className="count">{units.length}</b></div></div><div className="coordination-tree-head"><span>Об’єкт управління</span><span>Відповідальний</span><span>Статус</span><span>Прогрес</span><span>Строк</span><span>Увага / дія</span></div><div className="coordination-tree-body">{goals.map((goal) => renderCoordinationRow(goal))}{orphanCycles.map((cycle) => renderCoordinationRow(cycle))}{!units.length && <p className="empty-state padded">За вибраними фільтрами напрямів зусиль немає.</p>}</div></section>
  </>;
}
...
function ReportAnalytics({ payload, data, userById, healthFilter, setHealthFilter, query, ownerId, openWork }: { payload: PortalPayload; data: ComputedData; userById: (id: string) => PortalUser | undefined; healthFilter: "all" | HealthStatus | "overdue" | "decision"; setHealthFilter: (value: "all" | HealthStatus | "overdue" | "decision") => void; query: string; ownerId: string; openWork: (id: string) => void }) {
  const today = new Date().toISOString().slice(0, 10);
  const filtered = data.tasks.filter((node) => (healthFilter === "all" || node.health === healthFilter || (healthFilter === "overdue" && node.plannedEnd < today && !["idea", "completed", "cancelled"].includes(node.lifecycle)) || (healthFilter === "decision" && node.decisionRequired)) && (ownerId === "all" || node.acceptorId === ownerId || node.assigneeId === ownerId) && (!query.trim() || `${node.code} ${node.title} ${node.result}`.toLowerCase().includes(query.trim().toLowerCase())));
...
function AsanaSyncPanel({ payload, selected, asanaStatus, mutate, setNotice, compact = false }: { payload: PortalPayload; selected: WorkNode; asanaStatus: { configured: boolean; connected: boolean; connection?: Record<string, string> } | null; mutate: (action: string, entityId: string, recipe: (state: PortalState) => void) => Promise<boolean>; setNotice: Notify; compact?: boolean }) {
  const titles = useAsanaTitles(selected);
  ...
  const [busy, setBusy] = useState(false);
  const [outbox, setOutbox] = useState<{ pending: number; lastError: string } | null>(null);
  const linked = Boolean(selected.asana.taskGid);
  const expectedName = primaryAsanaTitle(selected);
  const titleMismatch = linked && asanaTitleDiffers(selected, selected.asana.remoteName || "");
  useEffect(() => {
    if (!linked) return;
    fetch(`/api/asana/outbox?nodeId=${encodeURIComponent(selected.id)}`, { cache: "no-store" })
      .then((response) => response.json() as Promise<{ pending: number; lastError: string }>)
      .then(setOutbox).catch(() => setOutbox(null));
  }, [linked, selected.id, selected.asana.lastSyncedAt, payload.revision]);
  const savedAsanaLinks = asanaLinks(selected.controlPlace, selected.description).filter((url) => url !== selected.asana.taskUrl);
  useEffect(() => {
  ...
  const sync = async (action: "read" | "create" | "update" | "rename", taskOverride = "") => {
    ...
    try {
      ...
      const result = (await response.json()) as { error?: string; workspaceGid?: string; tagWarning?: string; storySync?: { imported: number; partial: boolean; error?: string }; followerSync?: { added: number; skipped: string[] }; data?: { gid?: string; name?: string; notes?: string; due_on?: string | null; completed?: boolean; permalink_url?: string; modified_at?: string; assignee?: { email?: string; name?: string }; projects?: Array<{ gid: string; name: string }>; followers?: Array<{ gid: string; name: string; email?: string }> } };
      if (!response.ok) throw new Error(result.error || "Помилка Asana");
      const task = result.data || {};
      const saved = await mutate(`Синхронізовано ${selected.code} з Asana`, selected.id, (state) => {
            ...
            if (task.completed) {
              ...
              if (accepted) node.lifecycle = "completed";
                ...
                if (!submitted) {
                  ...
                  state.discussions.unshift({ id: crypto.randomUUID(), nodeId: node.id, authorId: payload.currentUser.id, recipientId: node.acceptorId, relatedType: "acceptance", relatedId: acceptanceId, asanaOriginated: true, text: "Asana підтвердила завершення пов’язаної задачі. Результат автоматично передано ініціатору на приймання.", kind: "approval", createdAt: isoNow() });
                }
              }
      ...
      setNotice(result.tagWarning || result.storySync?.error || (action === "create" ? `Головну задачу Asana створено й прив’язано${followerNote}` : action === "read" ? `Отримано стан та коментарі Asana: ${task.completed ? "завершено" : "активне"}${followerNote}${result.storySync?.partial ? " · частину давньої історії не завантажено" : ""}` : action === "rename" ? "Назву головної задачі Asana оновлено" : `Зміни передано в Asana${followerNote}`), result.tagWarning || result.storySync?.error ? "error" : undefined);
  ...
  const syncPanel = <section id={`asana-link-${selected.id}`} className={compact ? "work-section asana-work" : "panel sync-workbench"}>
    <div className="panel-head"><div><span>Головне джерело · Asana</span><h2>{linked ? "Задачу прив’язано" : "Прив’язати або створити задачу"}</h2></div><span className={`connection-state ${asanaStatus?.connected ? "connected" : ""}`}>{asanaStatus?.connected ? "Акаунт підключено" : "Акаунт не підключено"}</span></div>
    {savedAsanaLinks.length > 0 && <div className="asana-saved-links"><strong>Посилання, збережені в картці</strong>{savedAsanaLinks.map((url) => <div key={url}><a href={url} target="_blank" rel="noopener noreferrer">{asanaLinkLabel(url, titles[url])} ↗</a>{asanaStatus?.connected && !linked && <button disabled={busy} onClick={() => void sync("read", url)}>Перевірити й прив’язати</button>}</div>)}{!linked && <small>Посилання доступні для перегляду без підключення акаунта. Синхронізація вмикається окремо.</small>}</div>}
    {!asanaStatus?.connected ? <div className="asana-empty"><p>{linked ? "Посилання на задачу доступне нижче. Підключіть свій Asana-акаунт лише для оновлення й редагування задачі." : "Підключіть особистий Asana-акаунт у налаштуваннях лише для пошуку, створення та синхронізації задач."}</p>{linked && selected.asana.taskUrl && <a href={selected.asana.taskUrl} target="_blank" rel="noopener noreferrer">{asanaLinkLabel(selected.asana.taskUrl, titles[selected.asana.taskUrl] || selected.asana.remoteName)} ↗</a>}</div> : linked ? <div className="asana-linked-card"><div><span>Пов’язана задача</span><strong>{selected.asana.taskUrl ? <a className="asana-task-title" href={selected.asana.taskUrl} target="_blank" rel="noopener noreferrer">{asanaLinkLabel(selected.asana.taskUrl, titles[selected.asana.taskUrl] || selected.asana.remoteName)} ↗</a> : selected.asana.remoteName || selected.asana.taskGid}</strong><div className="asana-remote-state"><b className={!selected.asana.lastSyncedAt ? "pending" : selected.asana.remoteCompleted ? "completed" : "active"}>{!selected.asana.lastSyncedAt ? "Стан не оновлено" : selected.asana.remoteCompleted ? "Завершено в Asana" : "Активне в Asana"}</b>{selected.asana.remoteDueOn && <small>Строк {dateLabel(selected.asana.remoteDueOn)}</small>}{selected.asana.remoteAssignee && <small>{selected.asana.remoteAssignee}</small>}{Boolean(selected.asana.remoteFollowerCount) && <small>Фоловерів: {selected.asana.remoteFollowerCount}</small>}</div><small>{selected.asana.lastSyncedAt ? `Оновлено ${new Date(selected.asana.lastSyncedAt).toLocaleString("uk-UA")}` : "Натисніть «Оновити з Asana», щоб отримати фактичний стан"}</small></div><div><button disabled={busy} onClick={() => void sync("read")}>{busy ? "Оновлюємо…" : "Оновити з Asana"}</button><button disabled={busy} onClick={() => void sync("update")}>Передати зміни</button><button className="danger" disabled={busy} onClick={() => void unlink()}>Відв’язати</button></div></div> : <><div className="asana-mode-switch"><button className={mode === "link" ? "active" : ""} onClick={() => setDraft({ ...draft, mode: "link" })}>Прив’язати наявну</button><button className={mode === "create" ? "active" : ""} onClick={() => setDraft({ ...draft, mode: "create" })}>Створити нову</button></div>{mode === "link" ? <div className="asana-link-stack"><div className="asana-search-form"><label><span>Знайти завдання за назвою</span><input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void searchTasks(); } }} placeholder="Введіть частину назви…" /></label><button className="primary" disabled={searching || searchQuery.trim().length < 2} onClick={() => void searchTasks()}>{searching ? "Шукаю…" : "Знайти"}</button></div>{searchMessage && <p className="asana-search-message">{searchMessage}</p>}{searchResults.length > 0 && <div className="asana-search-results">{searchResults.map((task) => <article key={task.gid}><div><strong>{task.name}</strong><span>{task.projects?.map((project) => project.name).join(", ") || task.workspace?.name || "Без проєкту"}</span><small>{task.assignee?.name || "Без виконавця"} · {task.completed ? "Завершено" : "Активне"} · {task.due_on ? `до ${dateLabel(task.due_on)}` : "без строку"}</small></div><button disabled={busy} onClick={() => void sync("read", task.gid)}>Прив’язати</button></article>)}</div>}<div className="asana-manual-divider"><span>або вставте посилання</span></div><div className="asana-link-form"><label><span>Посилання або GID задачі Asana</span><input value={taskGid} onChange={(event) => setDraft({ ...draft, taskGid: event.target.value })} placeholder="https://app.asana.com/… або GID" /></label><button className="primary" disabled={busy || !taskGid.trim()} onClick={() => void sync("read")}>Перевірити й прив’язати</button></div></div> : <div className="asana-create-form"><label><span>Місце створення в Asana</span>{destinationField}</label><div><span>Буде створено</span><strong>{expectedName}</strong><small>Учасники картки будуть додані фоловерами; без проєкту задача з’явиться у «Моїх завданнях» поточного Asana-користувача.</small></div><button className="primary" disabled={busy || (!projectGid && !effectiveWorkspaceGid)} onClick={() => void sync("create")}>Створити в Asana й прив’язати</button></div>}</>}
    {linked && titleMismatch && <div className="asana-name-suggestion"><strong>Назви відрізняються</strong><p>Головна задача Asana: {selected.asana.remoteName}. Назва картки порталу: {expectedName}.</p><button disabled={busy || !asanaStatus?.connected} onClick={() => void sync("rename")}>Змінити назву в Asana на «{expectedName}»</button></div>}
    {linked && Boolean(outbox?.pending) && <div className="asana-delivery-notice"><strong>Очікує передання в Asana: {outbox?.pending}</strong><p>{outbox?.lastError || "Коментарі та звіти буде передано від імені їхніх авторів після підключення Asana."}</p><button disabled={busy || !asanaStatus?.connected} onClick={async () => { setBusy(true); try { const response = await fetch("/api/asana/outbox", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ nodeId: selected.id }) }); const result = await response.json() as { sent?: number; pending?: number; errors?: string[] }; if (!response.ok) throw new Error(result.errors?.[0] || "Помилка передання"); setOutbox({ pending: result.pending || 0, lastError: result.errors?.[0] || "" }); setNotice(result.errors?.[0] || `Передано в Asana: ${result.sent || 0}`, result.errors?.length ? "error" : undefined); } catch (error) { setNotice(error instanceof Error ? error.message : "Помилка передання", "error"); } finally { setBusy(false); } }}>Повторити передання моїх записів</button></div>}
    {asanaStatus?.connected && !asanaStatus.connection?.scope?.includes("stories:write") && <p className="asana-delivery-notice">Щоб увімкнути обмін коментарями й звітами, <a href="/api/asana/start">перепідключіть Asana</a> з новими дозволами.</p>}
...
function HealthOverrideFields({ node, setNode, error, clearError }: { node: WorkNode; setNode: (node: WorkNode) => void; error?: string; clearError: (key: keyof WorkNode) => void }) {
  const value = node.healthOverride || "automatic";
  return <><Field label="Ризик / блокер" required hint="Автоматично — стан без окремого ручного відхилення. Для ризику або блокера обов’язково додайте пояснення, яке бачитимуть у картці перегляду."><select value={value} onChange={(event) => { const next = event.target.value as "automatic" | HealthStatus; clearError("healthComment"); setNode({ ...node, healthOverride: next === "automatic" ? undefined : next, health: next === "automatic" ? "normal" : next, healthComment: next === "automatic" ? "" : node.healthComment || "" }); }}><option value="automatic">Автоматично</option><option value="risk">Є ризик</option><option value="blocked">Заблоковано</option></select></Field>{node.healthOverride && <Field wide label="Коментар до ризику / блокера" required error={error} hint="Поясніть факти, наслідок для результату та потрібну реакцію. Коментар показується в режимі перегляду картки."><textarea value={node.healthComment || ""} aria-invalid={Boolean(error)} onChange={(event) => { clearError("healthComment"); setNode({ ...node, healthComment: event.target.value }); }} placeholder="Що сталося, як це впливає на результат і яка реакція потрібна" /></Field>}</>;
}
...
function NodeModal({ node, setNode, nodes, users, errors, clearError, close, save }: { node: WorkNode; setNode: (node: WorkNode) => void; nodes: WorkNode[]; users: PortalUser[]; errors: NodeErrors; clearError: (key: keyof WorkNode) => void; close: () => void; save: () => void }) {
  ...
  return <ModalShell title={`${node.code} · ${node.title || "Новий об’єкт"}`} subtitle={kindLabels[node.kind]} close={close} footer={<><button onClick={close}>Скасувати</button><button className="primary" onClick={save}>Зберегти</button></>}>
    ...
    <div className="form-grid">
      ...
      <Field label="Тип" required hint="Після зміни типу портал автоматично визначає допустимий батьківський рівень і перший вільний код у новій гілці."><select value={node.kind} onChange={(event) => { const kind = event.target.value as NodeKind; const parentKinds = allowedParentKinds(kind); const currentParent = node.parentId ? nodes.find((item) => item.id === node.parentId) : undefined; const parent = currentParent && parentKinds.includes(currentParent.kind) ? currentParent : parentKinds.length ? nodes.find((item) => parentKinds.includes(item.kind) && !item.archived && item.id !== node.id) : undefined; clearError("parentId"); clearError("code"); setNode({ ...node, kind, parentId: parent?.id || null, code: nextCode(nodes, parent, node.id) }); }}><option value="goal">Стратегічна ціль</option><option value="cycle">Напрям зусиль</option><option value="subcycle">Проект</option><option value="task">Завдання</option></select></Field>
      {allowedParents.length > 0 && <Field wide required error={errors.parentId} label={`Батьківський рівень · ${parentKindsLabel(allowedParents)}`} hint={node.kind === "task" ? "Завдання можна включити безпосередньо в напрям зусиль або в його проект. Код автоматично зміниться за новим шляхом." : "Після вибору портал присвоїть перший вільний номер цього рівня в обраній гілці."}><select value={node.parentId || ""} aria-invalid={Boolean(errors.parentId)} onChange={(event) => { const parent = nodes.find((item) => item.id === event.target.value); clearError("parentId"); clearError("code"); setNode({ ...node, parentId: parent?.id || null, code: nextCode(nodes, parent, node.id) }); }}><option value="">Оберіть батьківський рівень</option>{validParents.map((item) => <option key={item.id} value={item.id}>{item.code} · {item.title}</option>)}</select></Field>}
      ...
      <div className="responsibility-fields"><Field label="Виконавець" required error={errors.assigneeId} hint="Людина, яка виконує завдання або координує нижчі рівні."><select value={node.assigneeId} aria-invalid={Boolean(errors.assigneeId)} onChange={(event) => update("assigneeId", event.target.value)}>{users.filter((user) => user.active).map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select></Field><Field label="Ініціатор" required error={errors.acceptorId} hint="Ініціює роботу, погоджує блокери, приймає рішення та результат або повертає його на доопрацювання."><select value={node.acceptorId} aria-invalid={Boolean(errors.acceptorId)} onChange={(event) => update("acceptorId", event.target.value)}>{users.filter((user) => user.active).map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select></Field></div>
      ...
      <Field label="Завершення" hint="Погоджений строк готового результату не може бути раніше поточного прогнозу."><input type="date" value={node.plannedEnd} onChange={(event) => update("plannedEnd", event.target.value)} />{node.forecastEnd && (!node.plannedEnd || node.forecastEnd > node.plannedEnd) && <small className="forecast-deadline-notice">Після збереження строк: {dateLabel(node.forecastEnd)}</small>}</Field>
      <Field label="Прогноз" hint="Якщо прогноз пізніше строку завершення, строк картки автоматично переноситься на дату прогнозу."><input type="date" value={node.forecastEnd} onChange={(event) => { const forecastEnd = event.target.value; clearError("forecastEnd"); setNode({ ...node, forecastEnd, plannedEnd: plannedEndForForecast(node.plannedEnd, forecastEnd) }); }} /></Field>
      <Field label="Прогрес, %" required hint={node.kind === "task" ? "Частка фактично отриманого результату, а не витраченого часу." : "Для цілі, напряму зусиль й проекту прогрес автоматично розраховується з нижчих рівнів."}><input type="number" min="0" max="100" value={node.progress} disabled={node.kind !== "task"} onChange={(event) => update("progress", Number(event.target.value))} /></Field>
      ...
      {node.kind === "cycle" && <><Field label="Перша координація" hint="Дата першого зведеного перегляду напряму зусиль; від неї календар розраховує наступні події."><input type="date" value={node.coordinationStartDate || ""} onChange={(event) => { const value = event.target.value; setNode({ ...node, coordinationStartDate: value, coordinationWeekday: value ? new Date(`${value}T12:00:00`).getDay() : node.coordinationWeekday }); }} /></Field><Field label="День координації" hint="Виберіть день тижня. Якщо першу дату вже задано, вона пересунеться вперед до обраного дня."><select value={node.coordinationWeekday ?? 1} onChange={(event) => { const weekday = Number(event.target.value); setNode({ ...node, coordinationWeekday: weekday, coordinationStartDate: alignDateToWeekday(node.coordinationStartDate || "", weekday) }); }}>{weekdayLabels.map((label, index) => <option key={label} value={index}>{label}</option>)}</select></Field><Field label="Періодичність, днів" hint="Кількість календарних днів між координаціями напряму зусиль." example="7 — щотижня; 14 — раз на два тижні"><input type="number" min="1" max="365" value={node.coordinationIntervalDays || 7} onChange={(event) => update("coordinationIntervalDays", Math.max(1, Number(event.target.value) || 1))} /></Field></>}
...
function BlockerModal({ node, users, currentUserId, notify, close, save }: { node: WorkNode; users: PortalUser[]; currentUserId: string; notify: Notify; close: () => void; save: (blocker: Blocker) => void }) {
  const [form, setForm, clearDraft] = usePersistentDraft(`portal:blocker-draft:${currentUserId}:${node.id}`, { title: "", facts: "", cause: "", actionsTaken: "", ownerId: node.assigneeId, escalationToId: node.acceptorId, recommendation: "", impact: "", decisionDue: node.plannedEnd });
  const [errors, setErrors] = useState<Record<string, string>>({});
  const update = (key: string, value: string) => { setErrors((current) => ({ ...current, [key]: "" })); setForm({ ...form, [key]: value }); };
  const submit = () => {
    const next = { title: !form.title.trim() ? "Вкажіть, що саме зупинило виконання." : "", facts: !form.facts.trim() ? "Зафіксуйте перевірювані факти без припущень." : "", ownerId: !form.ownerId ? "Оберіть відповідального за усунення." : "", escalationToId: !node.acceptorId ? "Оберіть адресата ескалації." : "", decisionDue: !form.decisionDue ? "Вкажіть строк реакції." : "" };
    const active = Object.fromEntries(Object.entries(next).filter(([, value]) => value));
...
function DecisionModal({ node, users, currentUserId, notify, close, save }: { node: WorkNode; users: PortalUser[]; currentUserId: string; notify: Notify; close: () => void; save: (decision: Decision) => void }) {
  const [form, setForm, clearDraft] = usePersistentDraft(`portal:decision-draft:${currentUserId}:${node.id}`, { question: "", options: "", recommendation: "", decisionOwnerId: node.acceptorId, dueDate: node.plannedEnd });
  const [errors, setErrors] = useState<Record<string, string>>({});
  const update = (key: keyof typeof form, value: string) => { setErrors((current) => ({ ...current, [key]: "" })); setForm({ ...form, [key]: value }); };
  const submit = () => { const next = { question: !form.question.trim() ? "Сформулюйте одне рішення, яке потрібно прийняти." : "", decisionOwnerId: !node.acceptorId ? "Оберіть уповноважену особу." : "", dueDate: !form.dueDate ? "Вкажіть строк рішення." : "" }; const active = Object.fromEntries(Object.entries(next).filter(([, value]) => value)); if (Object.keys(active).length) { setErrors(active); notify("Не вдалося надіслати запит рішення: перевірте виділені поля.", "error"); focusFirstError(); return; } clearDraft(); save({ id: crypto.randomUUID(), nodeId: node.id, ...form, decisionOwnerId: node.acceptorId, resolution: "", status: "requested", createdAt: isoNow(), decidedAt: "" }); };
  return <ModalShell title={`Запит рішення · ${node.code}`} subtitle="Управлінська ескалація" close={close} footer={<><button onClick={close}>Скасувати</button><button className="primary" onClick={submit}>Надіслати запит</button></>}><div className="form-grid"><Field wide label="Питання" required error={errors.question} hint="Одне чітке питання, яке потребує управлінського вибору." example="Чи погоджуємо публікацію варіанта А без нового відео?"><textarea value={form.question} aria-invalid={Boolean(errors.question)} onChange={(event) => update("question", event.target.value)} placeholder="Наприклад: який із двох варіантів затверджуємо?" /></Field><Field wide label="Варіанти" hint="Перелічіть реальні альтернативи та короткий наслідок кожної."><textarea value={form.options} onChange={(event) => update("options", event.target.value)} placeholder="Варіант А — ...; варіант Б — ..." /></Field><Field wide label="Рекомендація" hint="Який варіант ви рекомендуєте та чому."><textarea value={form.recommendation} onChange={(event) => update("recommendation", event.target.value)} placeholder="Рекомендую варіант А, тому що…" /></Field><Field label="Ініціатор — прийняття рішення" hint="Рішення приймає ініціатор цієї картки."><input readOnly value={users.find((user) => user.id === node.acceptorId)?.name || "Не визначено"} /></Field><Field label="Строк рішення" required error={errors.dueDate} hint="Дата, до якої рішення ще дозволяє зберегти план або мінімізувати наслідки."><input type="date" value={form.dueDate} aria-invalid={Boolean(errors.dueDate)} onChange={(event) => update("dueDate", event.target.value)} /></Field></div></ModalShell>;
}
...
function CoordinationModal({ node, payload, currentUserId, notify, close, save }: { node: WorkNode; payload: PortalPayload; currentUserId: string; notify: Notify; close: () => void; save: (snapshot: CoordinationSnapshot) => void }) {
  ...
  return <ModalShell title={`Координація напряму зусиль ${node.code}: ${node.title}`} subtitle="Зведений знімок усіх завдань напряму зусиль" close={close} footer={<><button onClick={close}>Скасувати</button><button className="primary" onClick={submit}>Зберегти координацію напряму зусиль</button></>}><div className="coordination-path" aria-label="Шлях координації">{path.map((item, index) => <span key={item.id}><b>{item.code}</b><small>{item.title}</small>{index < path.length - 1 && <i>›</i>}</span>)}</div><div className="snapshot-preview"><strong>До знімка напряму зусиль потраплять</strong><span>{subcycles.length} проектів</span><span>{tasks.length} завдань</span><span>{tasks.filter((task) => task.lifecycle === "completed").length} завершено</span><span>{tasks.filter((task) => task.health === "blocked").length} заблоковано</span><span>{tasks.filter((task) => task.lifecycle === "acceptance").length} на прийманні</span></div><div className="form-grid"><Field wide label="Зведений висновок за напрямом зусиль" required error={summaryError} hint="Підсумуйте результат усіх завдань напряму зусиль: що завершено, де відхилення, які проекти потребують уваги, які рішення необхідні." example="7 із 10 завдань завершено; проект P1.2 має блокер; потрібне рішення щодо бюджету до 20.08."><textarea value={summary} aria-invalid={Boolean(summaryError)} onChange={(event) => { setForm({ ...form, summary: event.target.value }); setSummaryError(""); }} placeholder="Зведений стан завдань, проектів, строків і відхилень" /></Field><Field wide label="Погоджені дії та рішення" hint="Фіксуйте домовленість як: хто — що робить — до якого строку." example="Валентина до 18.08 готує макет; Володимир того ж дня погоджує."><textarea value={agreements} onChange={(event) => setForm({ ...form, agreements: event.target.value })} placeholder="Наприклад: відповідальний — дія — строк" /></Field></div></ModalShell>;
}

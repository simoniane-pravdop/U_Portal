...
import { asanaLinks } from "./lib/asana-links";
  ...
  return <>
    ...
    <div className={`tree-workbench ${treeCompact ? "compact-tree" : ""} mobile-pane-${mobilePane}`} style={{ "--tree-nav-width": `${treeWidth}px` } as React.CSSProperties}>
      ...
      {selected ? <section key={selected.id} className="node-detail">
        ...
        {detailTab === "passport" && selected.evidence.length > 0 && <section className="tree-evidence"><h3>Докази результату · {selected.evidence.length}</h3>{selected.evidence.map((item) => <article key={item.id}><strong>{item.label}</strong><p><LinkedText value={item.value} /></p></article>)}</section>}
...
function WorkCardDescription({ node, payload, userById, expanded = false }: { node: WorkNode; payload: PortalPayload; userById: (id: string) => PortalUser | undefined; expanded?: boolean }) {
  ...
  const value = (text: string) => <LinkedText value={text.trim() || "Не визначено"} />;
  ...
  const renderCoordinationRow = (node: WorkNode, depth = 0): React.ReactNode => {
    ...
    return <div key={node.id} className={`coordination-tree-branch level-${node.kind}`}>
      <div className="coordination-tree-row" style={{ "--coord-depth": depth } as React.CSSProperties}>
        <div className="coordination-object-cell"><button className="coordination-expand" disabled={!hasChildren} onClick={() => toggleRow(node.id)} aria-label={opened ? "Згорнути рівень" : "Розгорнути рівень"}>{hasChildren ? opened ? "⌄" : "›" : "·"}</button><a className="coordination-object" href={cardHref(node, payload.currentUser)} target="_blank" rel="noopener noreferrer" title={`Відкрити ${node.code} у новій вкладці`}><span>{node.code}</span><strong>{node.title}</strong><small>{node.kind === "cycle" ? `${payload.nodes.find((item) => item.id === node.parentId)?.code || "Без цілі"} · ${coordinationCadenceLabel(node)}` : kindLabels[node.kind]}</small></a></div>

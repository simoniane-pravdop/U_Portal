...
function DiscussionPanel({ node, payload, mutate, notify, userById }: { node: WorkNode; payload: PortalPayload; mutate: (action: string, entityId: string, recipe: (state: PortalState) => void) => Promise<boolean>; notify: Notify; userById: (id: string) => PortalUser | undefined }) {
  const [draft, setDraft, clearDraft] = usePersistentDraft(`portal:discussion-draft:${payload.currentUser.id}:${node.id}`, { text: "", kind: "comment" as "comment" | "question", recipientId: node.acceptorId, requiresResponse: false });
  const [busy, setBusy] = useState(false);
  ...
  const effectiveRecipientId = draft.recipientId || node.acceptorId;
  const send = async () => {
  ...
  const mayChangeMessage = (message: DiscussionMessage) => ["comment", "question"].includes(message.kind) && (message.authorId === payload.currentUser.id || node.acceptorId === payload.currentUser.id || ["owner", "admin"].includes(payload.currentUser.role));

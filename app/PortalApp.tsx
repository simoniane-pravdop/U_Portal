...
function AsanaSyncPanel({ payload, selected, asanaStatus, mutate, setNotice, compact = false }: { payload: PortalPayload; selected: WorkNode; asanaStatus: { configured: boolean; connected: boolean; connection?: Record<string, string> } | null; mutate: (action: string, entityId: string, recipe: (state: PortalState) => void) => Promise<boolean>; setNotice: Notify; compact?: boolean }) {
  const [draft, setDraft, clearDraft] = usePersistentDraft(`portal:asana-draft:${selected.id}`, { taskGid: selected.asana.taskGid || "", projectGid: selected.asana.projectGid || "", mode: "link" as "link" | "create" });
  const { taskGid, projectGid, mode } = draft;
  ...
  const sync = async (action: "read" | "create" | "update") => {
    ...
    try {
      ...
      const response = await fetch("/api/asana/sync", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, nodeId: selected.id, taskGid: normalizedTaskGid, projectGid: resolvedProjectGid, title: selected.title, description: `${selected.code}\n\n${selected.result}\n\nКритерій приймання: ${selected.acceptanceCriteria}`, dueOn: selected.plannedEnd, completed: selected.lifecycle === "completed" }) });
      ...
      setDraft({ taskGid: task?.gid || normalizedTaskGid, projectGid: resolvedProjectGid, mode: "link" });
      clearDraft();
      setNotice(action === "create" ? "Задачу Asana створено й прив’язано" : action === "read" ? "Задачу Asana перевірено й прив’язано" : "Зміни передано в Asana");
  ...
  const unlink = async () => {
    ...
    if (ok) { setDraft({ taskGid: "", projectGid: "", mode: "link" }); clearDraft(); }
  };
  const projectField = asanaProjects.length ? <select value={projectGid} onChange={(event) => setDraft({ ...draft, projectGid: event.target.value })}><option value="">Оберіть проєкт Asana</option>{asanaProjects.map((project) => <option key={project.gid} value={project.gid}>{project.name} · {project.workspace}</option>)}</select> : <input value={projectGid} onChange={(event) => setDraft({ ...draft, projectGid: event.target.value.trim() })} placeholder="GID проєкту, якщо перелік недоступний" />;

...
export function PortalApp() {
  ...
  const submitAcceptance = (node: WorkNode) => mutate(`Передано ${node.code} на приймання`, node.id, (state) => {
    ...
    state.discussions.unshift({ id: crypto.randomUUID(), nodeId: node.id, authorId: payload.currentUser.id, recipientId: node.acceptorId, relatedType: "acceptance", relatedId: acceptance.id, text: `Результат передано на приймання: ${payload.users.find((user) => user.id === node.acceptorId)?.name || "приймальнику"}. Критерій: ${node.acceptanceCriteria || "не визначено"}.`, kind: "approval", createdAt: isoNow() });
  });
        ...
        <div className="content">
          ...
          {view === "tree" && (
            <TreeView
              ...
              mutate={mutate} completeNode={completeNode} copyNodeLink={copyNodeLink}
            />
          )}
    ...
    <div className="overview-grid">
      ...
      <section className="panel attention-panel">
        ...
        <div className="attention-list">
          {attention.slice(0, 6).map((node) => <button key={node.id} onClick={() => select(node.id)}><span className={`signal ${node.health}`} /><div><strong>{node.code} · {node.title}</strong><small>{node.health === "blocked" ? "Заблоковано" : node.health === "risk" ? "Є ризик виконання" : `Строк ${dateLabel(node.plannedEnd)}`}</small></div><b>→</b></button>)}
...
function TreeView(props: {
  ...
  openWork: (node: WorkNode) => void; mutate: (action: string, entityId: string, recipe: (state: PortalState) => void) => Promise<boolean>; completeNode: (node: WorkNode) => Promise<boolean>; copyNodeLink: (node: WorkNode, targetView: View) => Promise<void>;
}) {
  const { payload, selected, selectedId, setSelectedId, search, setSearch, userById, canManage, locks, openCreateKind, openEdit, openWork, mutate, completeNode, copyNodeLink } = props;
...
function NotificationsView({ payload, userById, mutate, openNode }: { payload: PortalPayload; userById: (id: string) => PortalUser | undefined; mutate: (action: string, entityId: string, recipe: (state: PortalState) => void) => Promise<boolean>; openNode: (id: string) => void }) {
...
function MyWork({ payload, selected, selectedId, setSelectedId, userById, canManage, saveWorkUpdate, submitAcceptance, resolveAcceptance, completeNode, openEdit, copyNodeLink, setModal, asanaStatus, mutate, setNotice, openTree }: { payload: PortalPayload; selected?: WorkNode; selectedId: string; setSelectedId: (id: string) => void; userById: (id: string) => PortalUser | undefined; canManage: boolean; saveWorkUpdate: (node: WorkNode, update: Omit<WorkUpdate, "id" | "createdAt" | "createdBy" | "source">) => Promise<boolean>; submitAcceptance: (node: WorkNode) => Promise<boolean>; resolveAcceptance: (acceptance: Acceptance, accepted: boolean, feedback?: string) => Promise<boolean>; completeNode: (node: WorkNode) => Promise<boolean>; openEdit: (node: WorkNode) => Promise<void>; copyNodeLink: (node: WorkNode, targetView: View) => Promise<void>; setModal: (modal: Modal) => void; asanaStatus: { configured: boolean; connected: boolean; connection?: Record<string, string> } | null; mutate: (action: string, entityId: string, recipe: (state: PortalState) => void) => Promise<boolean>; setNotice: Notify; openTree: (id: string) => void }) {
  const [filter, setFilter] = useState<"action" | "manage" | "inbox" | "all">("action");
  ...
  const filtered = mine.filter((node) => filter === "all" || filter === "action" && node.assigneeId === payload.currentUser.id && !["completed", "cancelled"].includes(node.lifecycle) || filter === "manage" && node.ownerId === payload.currentUser.id && node.assigneeId !== payload.currentUser.id || filter === "inbox" && incomingNodeIds.has(node.id))
  ...
  const nodeDecisions = current ? payload.decisions.filter((item) => item.nodeId === current.id).sort((a, b) => b.createdAt.localeCompare(a.createdAt)) : [];
      ...
      <aside className="work-inbox">
        <label className="work-object-picker compact">
          ...
          <select value={levelFilter} onChange={(event) => setLevelFilter(event.target.value as typeof levelFilter)}>
            ...
            <option value="goal">Усі цілі</option>
            <option value="cycle">Усі цикли</option>
            <option value="subcycle">Усі підцикли</option>
            <option value="task">Усі завдання</option>
          </select>
        </label>
        <div className="work-filter">{(["action", "manage", "inbox", "all"] as const).map((item) => <button key={item} className={filter === item ? "active" : ""} onClick={() => setFilter(item)}>{item === "action" ? "Мої дії" : item === "manage" ? "Координую" : item === "inbox" ? `Вхідні${incomingNodeIds.size ? ` · ${incomingNodeIds.size}` : ""}` : "Усі"}</button>)}</div>
...
function AsanaSyncPanel({ payload, selected, asanaStatus, mutate, setNotice, compact = false }: { payload: PortalPayload; selected: WorkNode; asanaStatus: { configured: boolean; connected: boolean; connection?: Record<string, string> } | null; mutate: (action: string, entityId: string, recipe: (state: PortalState) => void) => Promise<boolean>; setNotice: Notify; compact?: boolean }) {
  const [draft, setDraft, clearDraft] = usePersistentDraft(`portal:asana-draft:${selected.id}`, { taskGid: selected.asana.taskGid || "", projectGid: selected.asana.projectGid || "", workspaceGid: selected.asana.workspaceGid || "", mode: "link" as "link" | "create" });
...
function CoordinationModal({ node, payload, notify, close, save }: { node: WorkNode; payload: PortalPayload; notify: Notify; close: () => void; save: (snapshot: CoordinationSnapshot) => void }) {
  ...
  const submit = () => { if (!summary.trim()) { setSummaryError("Зафіксуйте висновок координації та потрібну управлінську увагу."); notify("Не вдалося зберегти координацію: заповніть виділене поле.", "error"); focusFirstError(); return; } clearDraft(); save({ id: crypto.randomUUID(), subcycleId: node.id, title: `${node.code}: ${node.title}`, path: path.map((item) => `${item.code}: ${item.title}`).join(" / "), date: isoNow().slice(0, 10), facilitatorId: payload.currentUser.id, summary, agreements, taskState: tasks.map((task) => ({ nodeId: task.id, lifecycle: task.lifecycle, health: task.health, plannedEnd: task.plannedEnd, progress: task.progress })), blockerIds: payload.blockers.filter((item) => item.status === "open" && tasks.some((task) => task.id === item.nodeId)).map((item) => item.id), decisionIds: payload.decisions.filter((item) => item.status === "requested" && tasks.some((task) => task.id === item.nodeId)).map((item) => item.id), createdAt: isoNow() }); };
  return <ModalShell title={`Координація ${node.code}: ${node.title}`} subtitle="Знімок фактичного стану" close={close} footer={<><button onClick={close}>Скасувати</button><button className="primary" onClick={submit}>Зберегти знімок</button></>}><div className="coordination-path" aria-label="Шлях координації">{path.map((item, index) => <span key={item.id}><b>{item.code}</b><small>{item.title}</small>{index < path.length - 1 && <i>›</i>}</span>)}</div><div className="snapshot-preview"><strong>До знімка потраплять</strong><span>{tasks.length} завдань</span><span>{tasks.filter((task) => task.health === "blocked").length} заблоковано</span><span>{tasks.filter((task) => task.lifecycle === "acceptance").length} на прийманні</span></div><div className="form-grid"><Field wide label="Управлінське резюме" required error={summaryError} hint="Короткий висновок зі стану одиниці координації: результат, відхилення, блокери та потрібна увага." example="2 із 4 завдань завершено; база знань заблокована; потрібне рішення щодо автора до 16.08."><textarea value={summary} aria-invalid={Boolean(summaryError)} onChange={(event) => { setForm({ ...form, summary: event.target.value }); setSummaryError(""); }} placeholder="Наприклад: що виконано, що відхилено і що потребує уваги" /></Field><Field wide label="Погоджені дії та рішення" hint="Фіксуйте домовленість як: хто — що робить — до якого строку." example="Валентина до 18.08 готує макет; Володимир того ж дня погоджує."><textarea value={agreements} onChange={(event) => setForm({ ...form, agreements: event.target.value })} placeholder="Наприклад: відповідальний — дія — строк" /></Field></div></ModalShell>;
}

...
export function PortalApp() {
  ...
  return (
    <main className="app-shell">
      ...
      {modal === "node" && draftNode && <NodeModal node={draftNode} setNode={setDraftNode} nodes={payload.nodes} users={payload.users} errors={nodeErrors} clearError={(key) => setNodeErrors((current) => ({ ...current, [key]: undefined }))} close={() => setModal(null)} save={saveNode} />}
      ...
      {modal === "coordination" && selected?.kind === "subcycle" && <CoordinationModal node={selected} payload={payload} notify={setNotice} close={() => setModal(null)} save={async (snapshot) => { const ok = await mutate(`Зафіксовано координацію ${selected.code}`, selected.id, (state) => state.coordinations.unshift(snapshot)); if (ok) setModal(null); }} />}
...
function MyWork({ payload, selected, selectedId, setSelectedId, userById, canManage, saveWorkUpdate, submitAcceptance, resolveAcceptance, setModal, asanaStatus, mutate, setNotice, openTree }: { payload: PortalPayload; selected?: WorkNode; selectedId: string; setSelectedId: (id: string) => void; userById: (id: string) => PortalUser | undefined; canManage: boolean; saveWorkUpdate: (node: WorkNode, update: Omit<WorkUpdate, "id" | "createdAt" | "createdBy" | "source">) => Promise<boolean>; submitAcceptance: (node: WorkNode) => Promise<boolean>; resolveAcceptance: (acceptance: Acceptance, accepted: boolean) => Promise<boolean>; setModal: (modal: Modal) => void; asanaStatus: { configured: boolean; connected: boolean; connection?: Record<string, string> } | null; mutate: (action: string, entityId: string, recipe: (state: PortalState) => void) => Promise<boolean>; setNotice: Notify; openTree: (id: string) => void }) {
...
function WorkStatusForm({ node, disabled, save, notify }: { node: WorkNode; disabled: boolean; save: (update: Omit<WorkUpdate, "id" | "createdAt" | "createdBy" | "source">) => Promise<boolean>; notify: Notify }) {
  ...
  const submit = async () => {
    ...
    if (ok) setForm((current) => ({ ...current, summary: "", nextAction: "" }));
    setBusy(false);
  };
...
function NodeModal({ node, setNode, nodes, users, errors, clearError, close, save }: { node: WorkNode; setNode: (node: WorkNode) => void; nodes: WorkNode[]; users: PortalUser[]; errors: NodeErrors; clearError: (key: keyof WorkNode) => void; close: () => void; save: () => void }) {
  ...
  return <ModalShell title={`${node.code} · ${node.title || "Новий об’єкт"}`} subtitle={kindLabels[node.kind]} close={close} footer={<><button onClick={close}>Скасувати</button><button className="primary" onClick={save}>Зберегти</button></>}>
    <div className="form-grid">
      ...
      <Field label="Прогрес, %" required hint="Частка фактично отриманого результату, а не витраченого часу."><input type="number" min="0" max="100" value={node.progress} onChange={(event) => update("progress", Number(event.target.value))} /></Field>
      ...
      <Field wide label="Контрольне місце" hint="Система або сторінка, де перевіряється фактичне виконання." example="Asana, CRM, сторінка сайту або реєстр договорів"><input value={node.controlPlace} onChange={(event) => update("controlPlace", event.target.value)} placeholder="Наприклад: Asana та сторінка сайту" /></Field>
...
function BlockerModal({ node, users, notify, close, save }: { node: WorkNode; users: PortalUser[]; notify: Notify; close: () => void; save: (blocker: Blocker) => void }) {
  ...
  const submit = () => {
    ...
    save({ id: crypto.randomUUID(), nodeId: node.id, ...form, status: "open", createdAt: isoNow(), resolvedAt: "" });
  };
...
function DecisionModal({ node, users, notify, close, save }: { node: WorkNode; users: PortalUser[]; notify: Notify; close: () => void; save: (decision: Decision) => void }) {
  ...
  const submit = () => { const next = { question: !form.question.trim() ? "Сформулюйте одне рішення, яке потрібно прийняти." : "", decisionOwnerId: !form.decisionOwnerId ? "Оберіть уповноважену особу." : "", dueDate: !form.dueDate ? "Вкажіть строк рішення." : "" }; const active = Object.fromEntries(Object.entries(next).filter(([, value]) => value)); if (Object.keys(active).length) { setErrors(active); notify("Не вдалося надіслати запит рішення: перевірте виділені поля.", "error"); focusFirstError(); return; } save({ id: crypto.randomUUID(), nodeId: node.id, ...form, resolution: "", status: "requested", createdAt: isoNow(), decidedAt: "" }); };
...
function CoordinationModal({ node, payload, notify, close, save }: { node: WorkNode; payload: PortalPayload; notify: Notify; close: () => void; save: (snapshot: CoordinationSnapshot) => void }) {
  ...
  const submit = () => { if (!summary.trim()) { setSummaryError("Зафіксуйте висновок координації та потрібну управлінську увагу."); notify("Не вдалося зберегти координацію: заповніть виділене поле.", "error"); focusFirstError(); return; } save({ id: crypto.randomUUID(), subcycleId: node.id, date: isoNow().slice(0, 10), facilitatorId: payload.currentUser.id, summary, agreements, taskState: tasks.map((task) => ({ nodeId: task.id, lifecycle: task.lifecycle, health: task.health, plannedEnd: task.plannedEnd, progress: task.progress })), blockerIds: payload.blockers.filter((item) => item.status === "open" && tasks.some((task) => task.id === item.nodeId)).map((item) => item.id), decisionIds: payload.decisions.filter((item) => item.status === "requested" && tasks.some((task) => task.id === item.nodeId)).map((item) => item.id), createdAt: isoNow() }); };
  return <ModalShell title={`Координація ${node.code}`} subtitle="Незмінний знімок фактичного стану" close={close} footer={<><button onClick={close}>Скасувати</button><button className="primary" onClick={submit}>Зберегти знімок</button></>}><div className="snapshot-preview"><strong>До знімка потраплять</strong><span>{tasks.length} завдань</span><span>{tasks.filter((task) => task.health === "blocked").length} заблоковано</span><span>{tasks.filter((task) => task.lifecycle === "acceptance").length} на прийманні</span></div><div className="form-grid"><Field wide label="Управлінське резюме" required error={summaryError} hint="Короткий висновок зі стану підциклу: результат, відхилення, блокери та потрібна увага." example="2 із 4 завдань завершено; база знань заблокована; потрібне рішення щодо автора до 16.08."><textarea value={summary} aria-invalid={Boolean(summaryError)} onChange={(event) => { setSummary(event.target.value); setSummaryError(""); }} placeholder="Наприклад: що виконано, що відхилено і що потребує уваги" /></Field><Field wide label="Погоджені дії та рішення" hint="Фіксуйте домовленість як: хто — що робить — до якого строку." example="Валентина до 18.08 готує макет; Володимир того ж дня погоджує."><textarea value={agreements} onChange={(event) => setAgreements(event.target.value)} placeholder="Наприклад: відповідальний — дія — строк" /></Field></div></ModalShell>;
}
...
function DependencyModal({ node, payload, notify, close, save }: { node: WorkNode; payload: PortalPayload; notify: Notify; close: () => void; save: (dependency: Dependency) => void }) {
  ...
  const submit = () => { if (!predecessorId) { setError("Немає вибраного допустимого попереднього завдання."); notify("Не вдалося додати залежність: перевірте виділене поле.", "error"); focusFirstError(); return; } save({ id: crypto.randomUUID(), predecessorId, successorId: node.id, type, createdAt: isoNow() }); };
  return <ModalShell title={`Залежність для ${node.code}`} subtitle="Мережа виконання" close={close} footer={<><button onClick={close}>Скасувати</button><button className="primary" onClick={submit}>Додати залежність</button></>}><div className="form-grid"><Field wide label="Попереднє завдання" required error={error} hint="Завдання, від події якого залежить початок або завершення поточного завдання."><select value={predecessorId} aria-invalid={Boolean(error)} onChange={(event) => { setPredecessorId(event.target.value); setError(""); }}>{candidates.length ? candidates.map((item) => <option key={item.id} value={item.id}>{item.code} · {item.title}</option>) : <option value="">Немає допустимих завдань</option>}</select></Field><Field wide label="Тип залежності" required hint="Визначає, яка подія попередника дозволяє або обмежує подію поточного завдання."><select value={type} onChange={(event) => setType(event.target.value as Dependency["type"])}><option value="finish_start">Завершення → початок</option><option value="start_start">Початок → початок</option><option value="finish_finish">Завершення → завершення</option><option value="start_finish">Початок → завершення</option></select></Field></div></ModalShell>;
}
...
function EvidenceModal({ node, currentUser, notify, close, save }: { node: WorkNode; currentUser: PortalUser; notify: Notify; close: () => void; save: (evidence: Evidence) => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      save({ id: crypto.randomUUID(), kind, label: label.trim(), value: finalValue, createdAt: isoNow(), createdBy: currentUser.id });
    } catch (cause) { notify(cause instanceof Error ? cause.message : "Не вдалося додати доказ", "error"); }
    finally { setBusy(false); }
  };
  return <ModalShell title={`Доказ виконання · ${node.code}`} subtitle="Перевірюваний результат" close={close} footer={<><button onClick={close}>Скасувати</button><button className="primary" disabled={busy} onClick={() => void submit()}>{busy ? "Завантажуємо…" : "Додати доказ"}</button></>}><div className="form-grid"><Field label="Тип" required hint="Посилання — на зовнішній результат; нотатка — коротка зафіксована перевірка; файл — матеріал, завантажений до порталу."><select value={kind} onChange={(event) => { setKind(event.target.value as Evidence["kind"]); setErrors({}); }}><option value="link">Посилання</option><option value="note">Нотатка</option><option value="file">Файл</option></select></Field><Field wide label="Назва доказу" required error={errors.label} hint="Назва має пояснювати, що підтверджує доказ." example="Опублікована сторінка послуги"><input value={label} aria-invalid={Boolean(errors.label)} onChange={(event) => { setLabel(event.target.value); setErrors((current) => ({ ...current, label: "" })); }} placeholder="Наприклад: погоджений макет сторінки" /></Field>{kind === "file" ? <Field wide label="Файл до 20 МБ" required error={errors.value} hint="Завантажте фінальний або контрольний матеріал, за яким можна перевірити результат."><input type="file" aria-invalid={Boolean(errors.value)} onChange={(event) => { setFile(event.target.files?.[0] || null); setErrors((current) => ({ ...current, value: "" })); }} /></Field> : <Field wide required error={errors.value} label={kind === "link" ? "Посилання" : "Зміст нотатки"} hint={kind === "link" ? "Вставте повне посилання на доступний результат." : "Стисло зафіксуйте факт, який підтверджує виконання."}>{kind === "link" ? <input value={value} aria-invalid={Boolean(errors.value)} onChange={(event) => { setValue(event.target.value); setErrors((current) => ({ ...current, value: "" })); }} placeholder="https://…" /> : <textarea value={value} aria-invalid={Boolean(errors.value)} onChange={(event) => { setValue(event.target.value); setErrors((current) => ({ ...current, value: "" })); }} placeholder="Наприклад: власник підтвердив приймання 14.08" />}</Field>}</div></ModalShell>;
}

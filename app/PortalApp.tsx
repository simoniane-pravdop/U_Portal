  ...
  const renderCoordinationRow = (node: WorkNode, depth = 0): React.ReactNode => {
    ...
    return <div key={node.id} className={`coordination-tree-branch level-${node.kind}`}>
      <div className="coordination-tree-row" style={{ "--coord-depth": depth } as React.CSSProperties}>
        <div className="coordination-object-cell"><button className={`coordination-expand ${hasChildren ? "has-children" : "is-leaf"}`} type="button" disabled={!hasChildren} aria-expanded={hasChildren ? opened : undefined} title={hasChildren ? `${opened ? "Згорнути" : "Розгорнути"} ${node.code}` : undefined} onClick={() => toggleRow(node.id)} aria-label={hasChildren ? `${opened ? "Згорнути" : "Розгорнути"} ${node.code}` : `${node.code} без нижчого рівня`}>{hasChildren ? opened ? "⌄" : "›" : ""}</button><a className="coordination-object" href={cardHref(node, payload.currentUser)} target="_blank" rel="noopener noreferrer" title={`Відкрити ${node.code} у новій вкладці`}><span>{node.code}</span><strong>{node.title}</strong><small>{node.kind === "cycle" ? `${payload.nodes.find((item) => item.id === node.parentId)?.code || "Без цілі"} · ${coordinationCadenceLabel(node)}` : kindLabels[node.kind]}</small></a></div>

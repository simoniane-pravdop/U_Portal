    ...
    <main className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
      ...
      <aside className="sidebar">
        ...
        <nav aria-label="Основна навігація">
          {nav.map((item) => (
            <button key={item.id} className={view === item.id ? "active" : ""} title={item.label} onClick={() => { if (item.id === "my") { setWorkEntryFilter("action"); setWorkEntryFocus(null); } setView(item.id); }}>
      ...
      {modal === "coordination" && selected?.kind === "cycle" && <CoordinationModal node={selected} payload={payload} notify={setNotice} close={() => setModal(null)} save={async (snapshot) => { const ok = await mutate(`Зафіксовано координацію циклу ${selected.code}`, selected.id, (state) => state.coordinations.unshift(snapshot)); if (ok) setModal(null); }} />}
...
function MyWork({ payload, selected, selectedId, setSelectedId, initialFilter, focusTarget, userById, canManage, saveWorkUpdate, resolveAcceptance, completeNode, openEdit, copyNodeLink, setModal, asanaStatus, mutate, setNotice, openTree }: { payload: PortalPayload; selected?: WorkNode; selectedId: string; setSelectedId: (id: string) => void; initialFilter: WorkFilter; focusTarget: WorkFocus; userById: (id: string) => PortalUser | undefined; canManage: boolean; saveWorkUpdate: (node: WorkNode, update: Omit<WorkUpdate, "id" | "createdAt" | "createdBy" | "source">) => Promise<boolean>; resolveAcceptance: (acceptance: Acceptance, accepted: boolean, feedback?: string) => Promise<boolean>; completeNode: (node: WorkNode) => Promise<boolean>; openEdit: (node: WorkNode) => Promise<void>; copyNodeLink: (node: WorkNode, targetView: View) => Promise<void>; setModal: (modal: Modal) => void; asanaStatus: { configured: boolean; connected: boolean; connection?: Record<string, string> } | null; mutate: (action: string, entityId: string, recipe: (state: PortalState) => void) => Promise<boolean>; setNotice: Notify; openTree: (id: string) => void }) {
  ...
  return <>
    {incomingNodeIds.size > 0 && <section className="approval-queue panel"><div className="panel-head"><div><span>Вхідні</span><h2>Блокери, приймання, рішення та питання</h2></div><b className="count amber">{pendingBlockers.length + pendingApprovals.length + pendingDecisions.length + pendingQuestions.length}</b></div><div>{pendingBlockers.map((blocker) => { const node = payload.nodes.find((item) => item.id === blocker.nodeId); return node ? <button key={blocker.id} onClick={() => { setSelectedId(node.id); setFilter("all"); window.setTimeout(() => document.getElementById(`manager-actions-${node.id}`)?.scrollIntoView({ behavior: "smooth", block: "start" }), 0); }}><span>Блокер · {node.code}</span><strong>{blocker.title}</strong><small>До {dateLabel(blocker.decisionDue)}</small></button> : null; })}{pendingApprovals.map((acceptance) => { const node = payload.nodes.find((item) => item.id === acceptance.nodeId); return node ? <button key={acceptance.id} onClick={() => { setSelectedId(node.id); setFilter("acceptance"); window.setTimeout(() => document.getElementById(`manager-actions-${node.id}`)?.scrollIntoView({ behavior: "smooth", block: "start" }), 0); }}><span>Приймання · {node.code}</span><strong>{node.title}</strong><small>Передано {new Date(acceptance.submittedAt).toLocaleString("uk-UA")}</small></button> : null; })}{pendingDecisions.map((decision) => { const node = payload.nodes.find((item) => item.id === decision.nodeId); return node ? <button key={decision.id} onClick={() => { setSelectedId(node.id); setFilter("all"); window.setTimeout(() => document.getElementById(`manager-actions-${node.id}`)?.scrollIntoView({ behavior: "smooth", block: "start" }), 0); }}><span>Рішення · {node.code}</span><strong>{decision.question}</strong><small>До {dateLabel(decision.dueDate)}</small></button> : null; })}{pendingQuestions.map((message) => { const node = payload.nodes.find((item) => item.id === message.nodeId); return node ? <button key={message.id} onClick={() => { setSelectedId(node.id); setFilter("all"); window.setTimeout(() => document.getElementById(`discussion-${node.id}`)?.scrollIntoView({ behavior: "smooth", block: "start" }), 0); }}><span>{message.kind === "question" ? "Питання" : "Коментар"} · {node.code}</span><strong>{message.text}</strong><small>Від {userById(message.authorId)?.name || "учасника"}</small></button> : null; })}</div></section>}
...
function BlockerApprovalCard({ blocker, resolve }: { blocker: Blocker; resolve: (blocker: Blocker, approved: boolean, comment: string) => Promise<boolean> }) {
  ...
  const [busy, setBusy] = useState(false);
  const act = async (approved: boolean) => { if (!approved && !comment.trim()) return; setBusy(true); await resolve(blocker, approved, comment); setBusy(false); };
  return <article className="blocker-approval-card"><header><span>Погодження блокера</span><strong>{blocker.title}</strong><time>до {dateLabel(blocker.decisionDue)}</time></header><dl><dt>Факти</dt><dd>{blocker.facts}</dd><dt>Рекомендація</dt><dd>{blocker.recommendation || "Не вказана"}</dd><dt>Наслідок без рішення</dt><dd>{blocker.impact || "Не вказаний"}</dd></dl><textarea value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Коментар, умови погодження або причина повернення…" /><div><button className="negative" disabled={busy || !comment.trim()} onClick={() => void act(false)}>Повернути з коментарем</button><button className="positive" disabled={busy} onClick={() => void act(true)}>Погодити реакцію</button></div></article>;
}
...
function ManagerActionCenter({ node, payload, latestAcceptance, openDecisions, userById, resolveAcceptance, resolveDecision, mutate }: { node: WorkNode; payload: PortalPayload; latestAcceptance?: Acceptance; openDecisions: Decision[]; userById: (id: string) => PortalUser | undefined; resolveAcceptance: (acceptance: Acceptance, accepted: boolean, feedback?: string) => Promise<boolean>; resolveDecision: (decision: Decision, resolution: string) => Promise<boolean>; mutate: (action: string, entityId: string, recipe: (state: PortalState) => void) => Promise<boolean> }) {
  ...
  const resolveBlocker = (blocker: Blocker, approved: boolean, comment: string) => mutate(approved ? `Погоджено реакцію на блокер ${node.code}` : `Повернуто реакцію на блокер ${node.code}`, node.id, (state) => {
    const target = state.blockers.find((item) => item.id === blocker.id)!;
    target.approvalStatus = approved ? "approved" : "returned";
    target.approvalComment = comment.trim() || target.recommendation;
    target.approvedBy = payload.currentUser.id;
    target.approvedAt = isoNow();
    state.discussions.unshift({ id: crypto.randomUUID(), nodeId: node.id, authorId: payload.currentUser.id, recipientId: blocker.ownerId, text: `${approved ? "Погоджено реакцію на блокер" : "Реакцію на блокер повернуто"}: ${target.approvalComment}`, kind: "approval", createdAt: isoNow() });
  });

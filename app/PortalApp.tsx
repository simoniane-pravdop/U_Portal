...
function workFilterForUser(node: WorkNode | undefined, user: PortalUser): WorkFilter {
  if (!node) return "all";
  if (node.ownerId === user.id) return "manage";
...
function hasWorkAccessForUser(node: WorkNode, user: PortalUser) {
  return node.ownerId === user.id || node.assigneeId === user.id || node.acceptorId === user.id || node.participantIds.includes(user.id);
}
...
function NodeModal({ node, setNode, nodes, users, errors, clearError, close, save }: { node: WorkNode; setNode: (node: WorkNode) => void; nodes: WorkNode[]; users: PortalUser[]; errors: NodeErrors; clearError: (key: keyof WorkNode) => void; close: () => void; save: () => void }) {
  ...
  return <ModalShell title={`${node.code} · ${node.title || "Новий об’єкт"}`} subtitle={kindLabels[node.kind]} close={close} footer={<><button onClick={close}>Скасувати</button><button className="primary" onClick={save}>Зберегти</button></>}>
    ...
    <div className="form-grid">
      ...
      <div className="responsibility-fields"><Field label="Координатор" required error={errors.ownerId} hint="Організовує виконання, контролює відхилення та забезпечує отримання результату."><select value={node.ownerId} aria-invalid={Boolean(errors.ownerId)} onChange={(event) => update("ownerId", event.target.value)}>{users.filter((user) => user.active).map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select></Field><Field label="Виконавець" required error={errors.assigneeId} hint="Людина, яка виконує завдання або координує нижчі рівні."><select value={node.assigneeId} aria-invalid={Boolean(errors.assigneeId)} onChange={(event) => update("assigneeId", event.target.value)}>{users.filter((user) => user.active).map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select></Field><Field label="Керівник вищої ланки" required error={errors.acceptorId} hint="Контролює об’єкт на вищому рівні та приймає результат або повертає його на доопрацювання."><select value={node.acceptorId} aria-invalid={Boolean(errors.acceptorId)} onChange={(event) => update("acceptorId", event.target.value)}>{users.filter((user) => user.active).map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select></Field></div>
